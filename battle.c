...
void battlePhaseUpdate( int64_t *unit, int64_t *results )
{
  ...
  {
    fa = (float) results[CMD_UNIT_CARRIER] / unit[CMD_UNIT_CARRIER];
    results[CMD_UNIT_BOMBER] += (int64_t) (fa * unit[CMD_UNIT_BOMBER]);
    results[CMD_UNIT_FIGHTER] += (int64_t)(fa * unit[CMD_UNIT_FIGHTER]);
    results[CMD_UNIT_TRANSPORT] += (int64_t)(fa * unit[CMD_UNIT_TRANSPORT]);
  }

  if( results[CMD_UNIT_TRANSPORT] )
  {
    fa = (float) results[CMD_UNIT_TRANSPORT] / unit[CMD_UNIT_TRANSPORT];
    results[CMD_UNIT_SOLDIER] += (int64_t)(fa * unit[CMD_UNIT_SOLDIER]);
    results[CMD_UNIT_DROID] += (int64_t)(fa * unit[CMD_UNIT_DROID]);
    results[CMD_UNIT_GOLIATH] += (int64_t)(fa * unit[CMD_UNIT_GOLIATH]);
  }
...
int battle( int id, int fltid, int64_t *results )
{
  ...
  int64_t popkilled = 0; //add pop defence

  int shields;
  int defsats, defsatsbase;
  int64_t hpcarrier, hptransport, hpcruiser, hpbomber, hpfighter, hpsoldier, hpdroid, hpgoliath, hpphantom, hptotal, hpsats, hppop;
  double damcarrier, damtransport, damcruiser, dambomber, damfighter, damsoldier, damdroid, damgoliath, damphantom, damsats, dampop;
...
/*** PHASE 3 ***/
  ...
  fa = (float)( attunit[CMD_UNIT_FIGHTER] ) / (float)( (defunit[CMD_UNIT_FIGHTER] == 0) ? 1 : defunit[CMD_UNIT_FIGHTER] );
  ...
  //ground fleet loss while transports were shot was forgotten, so added it 
  //transports shot
  /*int transportsShot =  results[4+4*CMD_UNIT_FLEET+CMD_UNIT_TRANSPORT];
  
  //average number of soldiers, droids and goliahs inside
  float avNumSoldiers = ((float) attunit[CMD_UNIT_SOLDIER])/ ((float)attunit[CMD_UNIT_TRANSPORT]);
  float avNumDroids = ((float) attunit[CMD_UNIT_DROID]) / ((float)attunit[CMD_UNIT_TRANSPORT]);
  float avNumGolaihs = ((float) attunit[CMD_UNIT_GOLIATH]) / ((float)attunit[CMD_UNIT_TRANSPORT]);
  
  results[4+4*CMD_UNIT_FLEET+CMD_UNIT_SOLDIER] = (int)(Math.floor(
  results[4+4*CMD_UNIT_FLEET+CMD_UNIT_DROID] = 
  results[4+4*CMD_UNIT_FLEET+CMD_UNIT_GOLIATH] =*/
...
/*** PHASE 4 ***/
  ...
  */
  defdam = defunit[CMD_UNIT_SOLDIER] * defstats[CMD_UNIT_SOLDIER][CMD_UNIT_STATS_GROUNDATTACK]  +  defunit[CMD_UNIT_DROID] * defstats[CMD_UNIT_DROID][CMD_UNIT_STATS_GROUNDATTACK]  +  defunit[CMD_UNIT_GOLIATH] * defstats[CMD_UNIT_GOLIATH][CMD_UNIT_STATS_GROUNDATTACK]  +  defunit[CMD_UNIT_PHANTOM] * defstats[CMD_UNIT_PHANTOM][CMD_UNIT_STATS_GROUNDATTACK] + (planetd.population / 10) * defstats[CMD_UNIT_SOLDIER][CMD_UNIT_STATS_GROUNDDEFENSE] ; //add pop defence;
	/*
  //ARTI CODE
  ...
  */
  ...
  if( ( ( defdam / attdam ) * 100.0 >= maind.config_flee[3] ) )
  {
    defdam = ( 0.40 * defdam );
    attdam = ( 0.20 * attdam );
    flee = 8;
  ...
  hppop = (planetd.population / 10) * defstats[CMD_UNIT_SOLDIER][CMD_UNIT_STATS_GROUNDDEFENSE]; //add pop defence

  hpphantom = defunit[CMD_UNIT_PHANTOM] * defstats[CMD_UNIT_PHANTOM][CMD_UNIT_STATS_GROUNDDEFENSE];
  hptotal = hpsoldier + hpdroid + hpgoliath + hpphantom + hppop; //add pop defence
  damsoldier = damdroid = damgoliath = damphantom = dampop =  0;
  if( hptotal )
  {
    damsoldier = (double)hpsoldier / (double)hptotal;
    damdroid = (double)hpdroid / (double)hptotal;
    damgoliath = (double)hpgoliath / (double)hptotal;
    damphantom = (double)hpphantom / (double)hptotal;
	dampop = (double)hppop / (double)hptotal; //add pop defence
  }
  ...
  fb = (float)(damsoldier + damdroid + damgoliath + damphantom + dampop); // add pop defence
  if( fb >= 0.00001 )
  {
    fa = attdam / fb;
    damsoldier *= fa;
  ...
  if( dampop > hppop )
    damsoldier += dampop - hppop; // add pop defence
  if( damsoldier > hpsoldier )
    damphantom += damsoldier - hpsoldier;
  if( damphantom > hpphantom )
    damdroid += damphantom - hpphantom;
  ...
  popkilled = (int64_t)(dampop * 10.0 / (defstats[CMD_UNIT_SOLDIER][CMD_UNIT_STATS_GROUNDDEFENSE])) ; // add pop defence
  //printf("%f %f %lld  %lld  %lld \n", dampop, defstats[CMD_UNIT_SOLDIER][CMD_UNIT_STATS_GROUNDDEFENSE], popkilled, planetd.population, planetd.population - popkilled);
 
 if (popkilled > planetd.population)
	 popkilled = planetd.population;
 
 planetd.population =  planetd.population - popkilled;
 
  popkilled = popkilled * 10 ; //somehow pop number is stored as divided by 10, this is to show the "real" ammount killed
	newd[30] = popkilled;
	popkilled2 = popkilled;
...
/*** PHASE 4 ***/
  ...
  defdam = defunit[CMD_UNIT_SOLDIER] * defstats[CMD_UNIT_SOLDIER][CMD_UNIT_STATS_GROUNDDEFENSE]  +  defunit[CMD_UNIT_DROID] * defstats[CMD_UNIT_DROID][CMD_UNIT_STATS_GROUNDDEFENSE]  +  defunit[CMD_UNIT_GOLIATH] * defstats[CMD_UNIT_GOLIATH][CMD_UNIT_STATS_GROUNDDEFENSE]  +  defunit[CMD_UNIT_PHANTOM] * defstats[CMD_UNIT_PHANTOM][CMD_UNIT_STATS_GROUNDDEFENSE] + (planetd.population / 10) * defstats[CMD_UNIT_SOLDIER][CMD_UNIT_STATS_GROUNDDEFENSE] ; //add pop defence
  ...
  newd[8+2*CMD_UNIT_FLEET] = defsatsbase - defsats;
	...
	fb *= (float)specopEnlightemntCalc(defid,CMD_ENLIGHT_BADFR);
  ...
  dbUserObservesSurveys_write(maind.empire, fleetd.destid, 1.0 ) ; 
   ...
   /*if(main2d.artefacts & ARTEFACT_16_BIT){ //add a random planet
    dbMainPlanetDef planetd3;
	newd[36] = exchange_planet( defid, planetd);
	dbMapRetrievePlanet( newd[36], &planetd3 );
	newd[37] = planetd3.position;
  }*/
