...
//static char dbFileUserRegistrationInfo[] = "%s/user%d/registerInfo";
...
dbFileUserMainName,
dbFileUserBuildName, 
dbFileUserPlanetsName, 
dbFileUserFleetsName, 
dbFileUserNewsName, 
dbFileUserMarketName, 
dbFileUserMailInName, 
dbFileUserMailOutName, 
dbFileUserSpecOpsName, 
dbFileUserRecordName, 
dbFileUserGameFlags
};
...
if( !( file = fopen( fname, "rb+" ) ) ) {
	...
	if( num < 0x10000 ) {
		printf("%s AND %s\n",COREDIR,fname);
		error( "Opening File: %s, user id %d", fname, id);
	}
...
if( !( file = fopen( fname, "rb+" ) ) ) {
	if( ( file = fopen( fname, "wb" ) ) ) {
		if (num != DB_FILE_EMPIRE_OBSERVES){
			file_w( dbFileEmpireListData[num], 1, dbFileEmpireListBase[num], file );
		}
		else{
			file_w( &i, sizeof(int), 1, file );
			file_s( file, sizeof(int) );
			file_w( &i, sizeof(int), 1, file );
		}
		fclose( file );
		return fopen( fname, "rb+" );
...
for( a = 0 ; a < b ; a++ ) {
	...
	strncpy( user->email, infod.email, sizeof(user->email) );
	strncpy( user->ip, infod.ip, sizeof(user->ip) );
...
int dbUserSearchEmail( char *email ) {
	dbUserPtr user;

for( user = dbUserList ; user ; user = user->next ) {
	if( ( ioCompareExact( email, user->email ) ) == 0 )
		continue;
...
file_s( dbFilePtr[DB_FILE_BASE_USERS], 4 ); //fseek(dbFilePtr[DB_FILE_BASE_USERS], 4, SEEK_SET) - from file begin
file_r( &freenum, 1, sizeof(int), dbFilePtr[DB_FILE_BASE_USERS] );
freenum = 0; //disable entering new users into deleted, comment out to enable
...
sprintf( user->email, "%s", adduser->email );
sprintf( user->ip, "%s", adduser->ip );
...
int dbUserPlanetListIndicesSortedBonus( int id, int **list, int sort )
{
  /*
	CMD_BONUS_ENERGY,
	CMD_BONUS_MINERAL,
	...
	CMD_BONUS_ECTROLIUM,
	CMD_BONUS_FISSION,
	CMD_BONUS_ANCIENTCIV,
	all
	none
	*/  
  sort -= 20;
  
  int a, b, c, d, num, numBonused = 0;
  int *listp, *listBonused = NULL, *listSort = NULL;
  dbMainPlanetDef planetd;
  FILE *file;
  
  //get all planets from file, then chose the ones with bonus
  ...
  for( a = 0 ; a < num ; a++ )
    {
      file_s( file, 4+(a*20) );
      file_r( &listp[a], 1, sizeof(int), file );
	  if( dbMapRetrievePlanet( listp[a], &planetd ) < 0 )
	    continue;
	  if (sort < 6 && (planetd.special[1] >0 || planetd.special[0] == CMD_BONUS_ANCIENTCIV)&& planetd.special[0] == sort)
		++numBonused;
	  else if(sort == 7 && !planetd.special[1] && planetd.special[0] != CMD_BONUS_ANCIENTCIV)
		++numBonused;
	  else if(sort == 6)
		++numBonused;
    }
	
    if( sort !=6 && !( listBonused = malloc( numBonused*sizeof(int)  ) ) )
    {
	  free(listp);
      fclose( file );
      return -1;
	...
	if( sort !=6 && !( listSort = malloc( numBonused*sizeof(int) ) ) )
    {
	  free(listBonused);
	  free(listp);
      fclose( file );
  ...
  if(sort != 6){
	  for(a = 0, b = 0; a < num; a++){
		if( dbMapRetrievePlanet( listp[a], &planetd ) < 0 )
			continue;
		if (sort < 6 && (planetd.special[1] >0 || planetd.special[0] == CMD_BONUS_ANCIENTCIV) && planetd.special[0] == sort){
			listBonused[b] = listp[a];
			for( c = d = 0 ; c < CMD_BLDG_NUMUSED ; c++ )
				d += planetd.building[c];
			listSort[b++]= (int)(100 * ( cmdGetBuildOvercost( planetd.size, d + planetd.construction ) -1 ) );
		}
		else if(sort == 7 && !planetd.special[1]  && planetd.special[0] != CMD_BONUS_ANCIENTCIV){
			listBonused[b] = listp[a];
			for( c = d = 0 ; c < CMD_BLDG_NUMUSED ; c++ )
  ...
  if (sort == 6){ 
	*list = listp; 
	 ...
	 sortlist( numBonused, listBonused, listSort );
	 *list = listBonused; 
	 free(listp);
	 free(listSort);
  }
...
// observed/surveyed planets
int dbUserObservesSurveys_size(int empire_id){
	FILE *file;
	int filesize = 0;
	
	if( !( file = dbFileEmpireOpen( empire_id, DB_FILE_EMPIRE_OBSERVES ) ) ) {
		//create
		error( "Opening Observes File " );
		return -3;
	}

	//1st 8 bytes store the ammount of elements
	file_r(&filesize, sizeof(int), 1, file);
	fclose(file);
	
	return filesize;
}

int dbUserObservesSurveys_read( int empire_id, void * plist ) {
	FILE *file;
	int filesize = 0;
	...
	if (filesize > 0)	
	{
		if (filesize > dbMapBInfoStatic[MAP_PLANETS]+1){
			error( "Too many systems, function dbUserObservesSurveys_read " );
			return 0;
		}
		file_s(file, sizeof(int)*2); //skip first two ints, first of which denotes number of file records
		file_r(plist, sizeof(int), (filesize)*2, file); //filesize *2 because each record stores planet Id and next is the strength of the observe
		fclose(file);
		return filesize;
	}
...
int dbUserObservesSurveys_search(int planet_id, float obs_strength, void * plist)
{
	int i, length, planet_id2;
	float obs_strength2;
	length = *(int*)plist;
	...
	for (i=2; i < 2*(length+1); i += 2)
	{
		...
		obs_strength2 = *((float*)plist + i + 1);
		
			//printf("OBS TEST length %d planet_id %d planet_id2 %d obs_strength %f obs_strength2 %f\n",length, planet_id, planet_id2, obs_strength, obs_strength2);
			
		if (planet_id2 == planet_id && obs_strength > obs_strength2) //planet exists in list but observe is stronger than previous
			{
			return i/2;
			}
		if (planet_id2 == planet_id && obs_strength <= obs_strength2) //planet exists in list but observe is weaker than previous
			{
			return -2;
			}	
	}
	
	return -1; //planet doesnt exist in the list yet
}

int dbUserObservesSurveys_write( int empire_id, int planet_id, float obs_strength ) {
	FILE *file;
	int filesize, temp, obsPlace;
	void * filecontent;
	...
	filecontent = malloc((filesize+1)*2*sizeof(int));
		
	rewind(file);
	file_r(filecontent, sizeof(int), (filesize+1)*2, file);
	
	obsPlace = filesize+1;
	++filesize;
	
	if (filesize>0){
		if ( ( temp = dbUserObservesSurveys_search(planet_id, obs_strength, filecontent) ) > 0 ){ //found a previous observe with weaker strength
		   obsPlace = temp;
		   --filesize;
		}
		else if (temp == -2){
	...
	//printf(" filesize%d TEMP%d\n",filesize,temp);
	
	//1st 8 bytes store the ammount of elements
		
	file_s(file, obsPlace*sizeof(int)*2);
	file_w(&planet_id, sizeof(int), 1, file);	
	file_s(file, obsPlace*sizeof(int)*2 + sizeof(int));
	fwrite(&obs_strength, sizeof(float), 1, file);
	
	rewind(file);
	file_w(&filesize, sizeof(int), 1, file);
...
int dbEmpireRelsReplace( int id, int relid, int *rel)
{
  int num;
  FILE *file;
  if( ( id < 0 ) || (id >= dbMapBInfoStatic[MAP_EMPIRES]) )
...
int dbEmpireRelsGet( int id, int relid, int *rel )
{
  ...
  //error("  id %d, relid %d ... %d  ", id, relid, (4+relid*sizeof(dbEmpireRelationsDef)) );
  file_s( file, 4+relid*sizeof(dbEmpireRelationsDef) );
