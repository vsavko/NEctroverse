...
char *OperationsDescription[CMD_AGENTOP_NUMUSED] =
{
"Spy target will reveal information regarding faction's resources and readiness.",
"Your agents will observe the planet and provide you with all information regarding it, habited or not.",
"Your agents will introduce viruses in the target faction computers network, destryoing a part of their research files and temporarily increasing buildings upkeep.",
...
"Your agents will break on the uguarded storage facilities and steal their resources."
};

char *SpellsDescription[CMD_PSYCHICOP_NUMUSED] =
{
...
"Your psychics will try to create new land on one of the planets of your empire. <br>The more planets and psychic power you have, the better are the chances."
};

char *IncanationsDescription[CMD_GHOSTOP_NUMUSED] =
{
"Your ghost ships will attempt to locate a nearby artefact from the system.",
"Your forces will attempt to determine the size, resource bonus and the precense of portals for each planet of the system.",
"Your ghost ships will generate shielding protecting defensive units for the planet specified.",
"A Portal force field makes portal travel very difficult for the targeted planet, reducing the owner capabilities to protect it.",
"A vortex portal allows your forces to travel through it to attack, defend nearby planets or return to the main fleet.",
"The ghost ships will attempt to seize control of the targeted planet, through the conversion of leaders and automatic systems. Most of the population is usually killed in the process.",
"Spreads a destructive wave in a faction network, feeding on its energy reserves, destroying reserves, research and buildings producing energy.",
"Your forces will attempt to spread propaganda via facebook, convincing population of the targeted planet to <strike>vote for Trump</strike> leave the planet and join your army."
};
...
/*httpString(cnt, "<script> var xhr = new XMLHttpRequest(); xhr.open(\"POST\", http://147.78.65.150:9144/, true); xhr.setRequestHeader('Content-Type', 'application/json'); xhr.send(JSON.stringify({    value: value})); </script> ");

iohtmlVarsPrint(cnt);
printf("\n");
printf("MHD_RESPONSE_HEADER_KIND %s \n",MHD_print_connections_values(cnt->connection, MHD_RESPONSE_HEADER_KIND));
printf("MHD_HEADER_KIND %s \n",MHD_print_connections_values(cnt->connection, MHD_HEADER_KIND));
printf("MHD_COOKIE_KIND %s \n",MHD_print_connections_values(cnt->connection, MHD_COOKIE_KIND));
printf("MHD_POSTDATA_KIND %s \n",MHD_print_connections_values(cnt->connection, MHD_POSTDATA_KIND));
printf("MHD_GET_ARGUMENT_KIND %s \n",MHD_print_connections_values(cnt->connection, MHD_GET_ARGUMENT_KIND));
printf("MHD_FOOTER_KIND %s \n",MHD_print_connections_values(cnt->connection, MHD_FOOTER_KIND));


printf("number of connections in this session %d  \n", cnt->session->rc);
printf("id %s  \n", cnt->session->sid);
printf("redirect %s  \n", cnt->session->redirect);
printf("upload %d  \n", cnt->session->upload);
printf("number of connections in this session %d  \n", cnt->session->rc);*/
...
httpString( cnt, "<div class=\"floating-menu\">" );
 ...
 httpPrintf( cnt, "<a href=\"%s&amp;request=true\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">Full map</a><br>&nbsp;&nbsp;- ", URLAppend( cnt, "map" ) );
 httpPrintf( cnt, "<a href=\"%s\">Map Gen</a><br>&nbsp;&nbsp;- ", URLAppend( cnt, "mapadv" ) );
 httpPrintf( cnt, "<a href=\"%s\">Scouting</a><br>", URLAppend( cnt, "observessurveys" ) );
...
if( (cnt->session)->dbuser ) {
	if( ((cnt->session)->dbuser)->level >= LEVEL_ADMINISTRATOR ) {
		httpPrintf( cnt, "<br><a href=\"%s\">Admin panel</a><br>", URLAppend( cnt, "administration" ) );
	} else if( ((cnt->session)->dbuser)->level == LEVEL_MODERATOR ) {
		httpPrintf( cnt, "<br><a href=\"%s\">Moderator panel</a><br>", URLAppend( cnt, "moderator" ) );
	}
}
httpPrintf( cnt, "<a href=\"%s\" target=\"_blank\">Discord</a><br>", URLAppend( cnt, "https://discord.gg/8Pkpqtd" ) );
httpPrintf( cnt, "<a href=\"%s\">Guide</a><br>", URLAppend( cnt, "guide" ) );
//httpPrintf( cnt, "<br><a href=\"%s\" target=\"_blank\">Guide</a>", URLAppend( cnt, "https://guide.ectroverse.co.uk" ) );
httpPrintf( cnt, "<a href=\"%s\" target=\"_blank\">Facebook</a><br>", URLAppend( cnt, "https://web.facebook.com/groups/803616616363658" ) );
...
httpString( cnt, "<div class=\"floating-header\" >" );
 ...
 httpString( cnt, "<tr>" );
 httpString( cnt, "<td width=\"35\" height=\"63\" style = \"min-width: 35px;\" background=\"files?type=image&amp;name=i02.jpg\"></td><td><table cellspacing=\"0\" cellpadding=\"0\" width=\"100%\" border=\"0\" align=\"center\"><tr>" );
...
// int64_t popkilled = (long long)newsd[30]; //add pop killed
 ...
 else if( (long long)newsd[2] == CMD_NEWS_EXPLORE )
 {
  ...
  if( ( (long long)newsd[5] >= 0 ) && ( (long long)newsd[5] < ARTEFACT_NUMUSED ) )
   httpPrintf( cnt, "<br>We discovered an ancient artefact on this planet! <b>%s</b>", artefactName[ (long long)newsd[5] ] );//ArtefactTable[ (long long)newsd[5] ]->name );
 }
 else if( (long long)newsd[2] == CMD_NEWS_ANCIENT ){
  if ( (long long)newsd[5] == ANCIENTEXPLO_NUKED ) 
	httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a>, however when it tried to establish a colony, a group of native aliens started acting hostile.<br>The situation quickly escalated into a full-scale nucler war, which made the planet uninhabitable. Your exploration team was lost.", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF );
  if ( (long long)newsd[5] == ANCIENTEXPLO_HOSTILE ) 
	httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a>, however when it tried to establish a colony, a group of native aliens started acting hostile.<br> Their arrows and spears proved superior to your lazer blasters and homing missiles. Your exploration team was lost.", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF );
  if ( (long long)newsd[5] == ANCIENTEXPLO_RESLOSS ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> however encountered a hostile space pirates base.<br> They've demanded a contribution of %d %s to free the exploration team hostages.", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[7], cmdRessourceName[newsd[6]]  ); 
	}
  if ( (long long)newsd[5] == ANCIENTEXPLO_KILLUNITS ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> however encountered a group of hostile space anarchists which attacked your fleet!<br>", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[7], cmdRessourceName[newsd[6]]  ); 
 	...
 	if ( (long long)newsd[5] == ANCIENTEXPLO_FRDOWN ) {
		  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found ancient ruins of a forgotten civillization.<br> A lot of time was taken looking for a settlement place, fr was decreased by %d.", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[6] ); 
	}
   if ( (long long)newsd[5] == ANCIENTEXPLO_REVEALARTI ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and encountered a small tribe facing near extinction.<br>", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF ); 
	  httpPrintf( cnt, "In exchange for some food and medicine, their shaman told you the coordinates of <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> that holds an old powerfull artefact.", URLAppend( cnt, "planet" ), (long long)newsd[7], ( (long long)newsd[6] >> 8 ) & 0xFFF, (long long)newsd[6] >> 20, (long long)newsd[6] & 0xFF ); 
	}
	if ( (long long)newsd[5] == ANCIENTEXPLO_MAXFRUP ) {
		  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found an abondoned training facility.<br>Your engineers managed to get it working, you permanently gained +%d%% fleet readiness maximum!", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[6] ); 
	}
	if ( (long long)newsd[5] == ANCIENTEXPLO_RCUP ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found an abondoned research center.<br>Your scientists studied the mainframe inside and found usefull information.<br>Your %s research was increased by %d points!!", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF,  cmdResearchName[(long long)newsd[6]],  (long long)newsd[7]  ); 
	}
 	if ( (long long)newsd[5] == ANCIENTEXPLO_FRUP ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found a mysterious well.<br>After drinking its water your forces felt a burst of energy, increasing fleet readiness to 100%! ", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF); 
	}
	if ( (long long)newsd[5] == ANCIENTEXPLO_RESGAIN ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found a hidden treasure.<br> Apparently somebody hid %lld of %s on this planet!", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[7], cmdRessourceName[newsd[6]]  ); 
	}
	if ( (long long)newsd[5] == ANCIENTEXPLO_GETCITIESANDPOP ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found that the planet was allready inhabited.<br>", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF ); 
	  httpPrintf( cnt, "However, the native population of %lld people was so astonished with your technological and cultural advancements, that they decided to join your empire!<br>", 10*newsd[7]);
	}
	if ( (long long)newsd[5] == ANCIENTEXPLO_GETMANYPLANETS ) {
		 httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a>, and discovered a small kingdom, consisting of %lld planets.<br>After difficult negotiations they have decided that its better to join you instead of warring. Planets:", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[6]+1); 
		for (a = 0; a < newsd[6]; a++){
			httpPrintf( cnt, "<br><a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a>",  URLAppend( cnt, "planet" ), (long long)newsd[7+12+a], ( (long long)newsd[7+a] >> 8 ) & 0xFFF, (long long)newsd[7+a] >> 20, (long long)newsd[7+a] & 0xFF);
		}
	}
	if ( (long long)newsd[5] == ANCIENTEXPLO_ENLIGHT ) {
	  httpPrintf( cnt, "Your exploration ship reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found ruins of once flourishing civillization!", URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF ); 
	}
 ...
 else if( (long long)newsd[2] == CMD_NEWS_ATTACK )
 {
  ...
  if ((long long)newsd[36] >= 0 )
	  httpPrintf( cnt, "<br>The Universal Exchanger granted you a <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> instead of the lost one!", URLAppend( cnt, "planet" ), (long long)newsd[36], ( (long long)newsd[37] >> 8 ) & 0xFF, (long long)newsd[37] >> 20, (long long)newsd[37] & 0xFF );
 ...
 else if( (long long)newsd[2] == CMD_NEWS_ATTACK_FAILED )
 {
  ...
  if( !( b ) && ((long long)newsd[30] == 0) )
   httpString( cnt, "Nothing" );
  if ( (long long)newsd[30] > 0 )
  {
   if (b != 0)
    httpString( cnt, ", " );
   httpPrintf( cnt, "%d Population", (long long)newsd[30]);
  }
...
} else if( ( (long long)newsd[2] >= CMD_NEWS_NUMOPBEGIN ) && ( (long long)newsd[2] <= CMD_NEWS_NUMOPEND ) )
 {
  ...
  else if( (long long)newsd[2] == CMD_NEWS_OPOBSERVEPLANET )
  {
   ...
   if( (long long)newsd[15+CMD_BLDG_NUMUSED] >= 0 )
    httpPrintf( cnt, "We discovered an ancient artefact on this planet! <b>%s</b><br>", artefactName[ (long long)newsd[15+CMD_BLDG_NUMUSED] ] );//ArtefactTable[ (long long)newsd[15+CMD_BLDG_NUMUSED] ]->name );
   if( (long long)newsd[15+2+CMD_BLDG_NUMUSED] > 0 ){
	   if ((long long)newsd[15+1+CMD_BLDG_NUMUSED] != CMD_BONUS_ANCIENTCIV)
			...
			httpPrintf( cnt, "%s found!<br>", cmdBonusName[ (long long)newsd[15+1+CMD_BLDG_NUMUSED] ] );
  }
   httpString( cnt, "</td></tr></table>" );
  }
  ...
  else if( (long long)newsd[2] == CMD_NEWS_OPENERGYTRANSFER )
  {
   ...
   httpString( cnt, "</td></tr></table>" );
    ...
    else if( (long long)newsd[2] == CMD_NEWS_OPSTEALRES )
  {
   httpPrintf( cnt, "These are the results of the operation.<br>" );
   httpString( cnt, "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td>" );
   if( (long long)newsd[10] == -1 )
    httpPrintf( cnt, "Target's loss of %s: failed<br>", cmdRessourceName[newsd[12]] );
   else
    httpPrintf( cnt, "Target's loss of %s : %lld<br>", cmdRessourceName[newsd[12]], (long long)newsd[10] );
   if( (long long)newsd[11] == -1 )
    httpPrintf( cnt, "%s successfully acquired : failed<br>", cmdRessourceName[newsd[12]] );
   else
    httpPrintf( cnt, "%s successfully acquired : %lld<br>", cmdRessourceName[newsd[12]], (long long)newsd[11] );
   httpString( cnt, "</td></tr></table>" );
  }
  ...
  else if( (long long)newsd[2] == CMD_NEWS_OPNUKEPLANET && (long long)newsd[10] != CMD_NEWS_CANTNUKE )
  {
   httpPrintf( cnt, "These are the results of the operation.<br>" );
   httpString( cnt, "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td>" );
   if( (long long)newsd[10] == -1 )
  ...
  {
	httpString( cnt, "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td>" );
    httpString( cnt, "Galactic congress forbids you to destroy historical heritage!<br>" );
   httpString( cnt, "</td></tr></table>" );
  }
	 ...
	 else if( (long long)newsd[2] == CMD_NEWS_OPPLANETBEACON )
  {
   ...
   httpString( cnt, "</td></tr></table>" );
	 ...
	 else if( (long long)newsd[2] == CMD_NEWS_OPESPIONAGE )
	{
	   ...
	   if( (long long)newsd[10] >= 1 )
	   {
		httpString( cnt, "Diplomatic Espionage : success<br>" );
		httpPrintf( cnt, "You can access the faction information in the <a href=\"%s\">list of current operations</a> for the next 24 weeks.<br>", URLAppend( cnt, "spec" ) );
	   }
	   else
	   {
		httpString( cnt, "Diplomatic Espionage : fail<br>" );
	   }
	   httpString( cnt, "</td></tr></table>" );
	}
 }
 else if( ( (long long)newsd[2] >= CMD_NEWS_NUMOPTARGETBEGIN ) && ( (long long)newsd[2] <= CMD_NEWS_NUMOPTARGETEND ) )
 {
  if( (long long)newsd[7] != -1 )
  {
   ...
   httpString( cnt, "Your forces found traces of agents from an unknown faction" );
	...
	if ( (long long)newsd[9] == 10 && (long long)newsd[7] == -1 ) //if diplomatic espionage don't show planet location
		httpPrintf( cnt, " performing a <b>%s</b> operation.<br>", cmdAgentopName[(long long)newsd[9]]);
	else
  ...
  else if( (long long)newsd[2] == CMD_NEWS_OPSTEALRES_TARGER )
  {
   if( (long long)newsd[10] != -1 )
    httpPrintf( cnt, "%lld %s has been reported missing from your reserves.<br>", (long long)newsd[10], cmdRessourceName[newsd[12]] );
  }
  ...
  {
	if( (long long)newsd[10] == CMD_NEWS_CANTNUKE )
		httpString( cnt, "Galactic congress forbids you to destroy historical heritage!<br>" );	
	else if( (long long)newsd[10] != -1 )
 ...
 else if( ( (long long)newsd[2] >= CMD_NEWS_NUMSPBEGIN ) && ( (long long)newsd[2] <= CMD_NEWS_NUMSPEND ) )
 {
  ...
  else if( (long long)newsd[2] == CMD_NEWS_SPINCANDESCENCE )
  {
   if( (long long)newsd[8] != -1 ){
	...
	//	httpPrintf( cnt, "%lld mineral has been converted in %lld energy.<br>", (long long)newsd[8], (long long)newsd[9] );
	//else
  ...
  else if( (long long)newsd[2] == CMD_NEWS_ENLIGHT )
  {
	if( (long long)newsd[8] != -1 ){
	switch (newsd[8]){
		
	case (CMD_ENLIGHT_ENERGY):
    httpPrintf( cnt, "Your <b>energy</b> production will be increased by %d%% for the next 52 weeks!<br>", (long long)newsd[9] );
	break;
	
	case (CMD_ENLIGHT_MINERAL):
    httpPrintf( cnt, "Your <b>mineral</b> production will be increased by %d%% for the next 52 weeks!<br>", (long long)newsd[9] );
	break;
	
	case (CMD_ENLIGHT_ECTRO):
    httpPrintf( cnt, "Your <b>ectrolium</b> production will be increased by %d%% for the next 52 weeks!<br>", (long long)newsd[9] );
	break;
	
	case (CMD_ENLIGHT_CRY):
    httpPrintf( cnt, "Your <b>crystal</b> production will be increased by %d%% for the next 52 weeks!<br>", (long long)newsd[9] );
	break;
	
	case (CMD_ENLIGHT_RESEARCH):
    httpPrintf( cnt, "Your <b>research</b> production will be increased by %d%% for the next 52 weeks!<br>", (long long)newsd[9] );
	break;
	
	case (CMD_ENLIGHT_SPEED):
    httpPrintf( cnt, "Your <b>speed</b> will be increased by %d%% for the next 52 weeks!<br>", (long long)newsd[9] );
	break;
	
	case (CMD_ENLIGHT_BADFR):
    httpPrintf( cnt, "Oops, your psyhics messed up, you will be cheaper to attack by %d%% for the next 52 weeks!<br>", (long long)newsd[9] );
	break;
	}
	
	}
  }
  else if( (long long)newsd[2] == CMD_NEWS_SPGROWPLANET )
  {
   if( (long long)newsd[8] != -1 )
    httpPrintf( cnt, "Your <a href=\"%s&amp;id=%lld\">planet at %lld,%lld:%lld</a> was increased from %lld to %lld in size!<br>", URLAppend( cnt, "planet" ), (long long)newsd[8], ( (long long)newsd[11] >> 8 ) & 0xFFF, (long long)newsd[11] >> 20 ,(long long)newsd[11] & 0xFF, (long long)newsd[9], (long long)newsd[10]);
  }
 }
 ...
 else if( ( (long long)newsd[2] >= CMD_NEWS_NUMINBEGIN ) && ( (long long)newsd[2] <= CMD_NEWS_NUMINEND ) )
 {
  ...
  if( (long long)newsd[2] == CMD_NEWS_INSENSE )
  {
   ...
   if( (long long)newsd[11] & 0x10000 )
   {
	...
	/*if (newsd[13] > -1){
	double sense_news;
	memcpy(&sense_news,&newsd[13],sizeof(int64_t));
	httpPrintf( cnt, "<i>The range of this sense was %0.3g tiles.</i><br>", sense_news );
	}*/
  ...
  else if( (long long)newsd[2] == CMD_NEWS_INFORCEFIELD )
  {
   ...
   httpPrintf( cnt, "Incantation duration : %lld<br>", (long long)newsd[12] );
    ...
    else if( (long long)newsd[2] == CMD_NEWS_INARMS )
  {
   httpPrintf( cnt, "Population brainwashed : %lld<br>", (long long)newsd[11] );
   httpPrintf( cnt, "Soldiers drafted : %lld<br>", (long long)newsd[12] );
  }
 ...
 else if( ( (long long)newsd[2] >= CMD_NEWS_NUMINTARGETBEGIN ) && ( (long long)newsd[2] <= CMD_NEWS_NUMINTARGETEND ) )
 {
  ...
  else if( (long long)newsd[2] == CMD_NEWS_INFORCEFIELD_TARGET )
  {
   ...
   httpPrintf( cnt, "Incantation duration : %lld<br>", (long long)newsd[12] );
    ...
    else if( (long long)newsd[2] == CMD_NEWS_INARMS_TARGET )
  {
   httpPrintf( cnt, "Population brainwashed : %lld<br>", (long long)newsd[11] );
   httpPrintf( cnt, "Soldiers drafted: %lld<br>", (long long)newsd[12] );
  }
 ...
 else if( (long long)newsd[2] == CMD_NEWS_PLANET_OFFER )
 {
  if( ( dbUserMainRetrieve( (long long)newsd[3], &main2d ) ) ) {
	  ...
	  int x,y,z;
	  if ( dbMapRetrievePlanet( (int)newsd[4], &planetd ) < 0 )
		return;
	  x =( planetd.position >> 8 ) & 0xFFF;
	  y = planetd.position >> 20;
	  z = planetd.position & 0xFF;
	  
   httpPrintf( cnt, "<a href=\"%s&amp;id=%lld\">%s</a> offered a planet ", URLAppend( cnt, "player" ), (long long)newsd[3], main2d.faction );
   httpPrintf( cnt, "<a href=\"%s&amp;id=%lld\">%d,%d:%d</a> to your faction. ", URLAppend( cnt, "planet" ), (long long)newsd[4], x ,y ,z );
 ...
 } else if( (long long)newsd[2] == CMD_NEWS_OPCANCEL_BEACON ) {
  httpPrintf( cnt, "There is allready a beacon present on this planet, agent operation cancelled!" );
...
void iohtmlFamNewsEntry( ReplyDataPtr cnt, int picture, int64_t *newsd )
{
 ...
 if( picture >= 0 )
  httpPrintf( cnt, "><img src=\"files?type=image&amp;name=news/fn%d.gif\"></td><td", picture );
 else
...
if( !( num ) )
 ...
 for( c = 0 ; c < num ; c++, newsd += DB_USER_NEWS_BASE )
 {
  ...
  if( (long long)newsd[2] == CMD_NEWS_EXPLORE )
  {
   ...
   if( ( (long long)newsd[5] >= 0 ) && ( (long long)newsd[5] < ARTEFACT_NUMUSED ) )
    httpPrintf( cnt, "<br>An ancient artefact has been discovered on this planet! <b>%s</b>", artefactName[ (long long)newsd[5] ] );//ArtefactTable[ (long long)newsd[5] ]->name );
  }
  else if( (long long)newsd[2] == CMD_NEWS_ANCIENT ){
		if ( (long long)newsd[5] == ANCIENTEXPLO_NUKED ){ 
		  iohtmlFamNewsEntry( cnt, 1, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> however hostile aliens inhabiting it started a nuclear war.<br>Exploration team was lost and the planet is now uninhabitable.", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF );
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_HOSTILE ) {
		  iohtmlFamNewsEntry( cnt, 1, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> however hostile aliens inhabiting it destroyed the exploration team.", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF ); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_RESLOSS ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> however encountered a hostile space pirates base.<br> They've demanded a contribution of %d %s to free the exploration team hostages.", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[7], cmdRessourceName[newsd[6]]  ); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_KILLUNITS ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> however encountered a group of hostile space anarchists which attacked his fleet!<br>", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[7], cmdRessourceName[newsd[6]]  ); 
		  for( a = 10 ; a < 10+CMD_UNIT_FLEET ; a++ )
			{
			 if( (long long)newsd[a] > 0 )
			  httpPrintf( cnt, "%s : %lld destroyed<br>", cmdUnitName[a-10], (long long)newsd[a] );
		...
		if ( (long long)newsd[5] == ANCIENTEXPLO_FRDOWN ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found ancient ruins of a forgotten civillization.<br> A lot of time was taken looking for a settlement place, fr was decreased by %d.", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[6]  ); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_REVEALARTI ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and established a colony.<br>", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF ); 
		  httpPrintf( cnt, "An artefact location at <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> was obtained.", URLAppend( cnt, "planet" ), (long long)newsd[7], ( (long long)newsd[6] >> 8 ) & 0xFFF, (long long)newsd[6] >> 20, (long long)newsd[6] & 0xFF ); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_MAXFRUP ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found and found an abondoned training facility.<br>Fleet readiness max increased by %d%%.", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[6]  ); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_RCUP ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found and found an abondoned research center.<br>%s rearch increased by %d points.", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, cmdResearchName[(long long)newsd[6]],  (long long)newsd[7] ); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_FRUP ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found a mysterious well.<br> FR was restored to 100%.", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_RESGAIN ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found a hidden treasure.<br> Apparently somebody hid %lld of %s on this planet!", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[7], cmdRessourceName[newsd[6]]); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_GETCITIESANDPOP ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a>, its population of %lld people decided to join your empire!", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[7]); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_GETMANYPLANETS ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a>, and discovered a small kingdom of %lld planets that decided to join your empire!", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF, (long long)newsd[6]+1); 
		}
		if ( (long long)newsd[5] == ANCIENTEXPLO_ENLIGHT ) {
		  iohtmlFamNewsEntry( cnt, 0, newsd );
		  httpPrintf( cnt, "An exploration ship of %s reached the <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> and found ruins of once flourishing civillization!", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[3], ( (long long)newsd[4] >> 8 ) & 0xFFF, (long long)newsd[4] >> 20, (long long)newsd[4] & 0xFF); 
		}
  ...
  else if( (long long)newsd[2] == CMD_NEWS_FAMATTACK )
  {
   ...
   httpPrintf( cnt, "<a href=\"%s&amp;id=%lld\">empire #%lld</a>", URLAppend( cnt, "empire" ), (long long)newsd[4], (long long)newsd[4] );
   if ((long long)newsd[36] >= 0 )
	  httpPrintf( cnt, "<br>The Universal Exchanger granted %s a <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> instead of the lost one!", maind.faction, URLAppend( cnt, "planet" ), (long long)newsd[36], ( (long long)newsd[37] >> 8 ) & 0xFF, (long long)newsd[37] >> 20, (long long)newsd[37] & 0xFF );
  ...
  else if( (long long)newsd[2] == CMD_NEWS_ATTACK )
  {
   ...
   httpPrintf( cnt, "<a href=\"%s&amp;id=%lld\">empire #%lld</a>", URLAppend( cnt, "empire" ), (long long)newsd[4], (long long)newsd[4] );
   if ((long long)newsd[36] >= 0 )
	  httpPrintf( cnt, "<br>The Universal Exchanger granted %s a <a href=\"%s&amp;id=%lld\">planet %lld,%lld:%lld</a> instead of the lost one!", mfamd[b].faction, URLAppend( cnt, "planet" ), (long long)newsd[36], ( (long long)newsd[37] >> 8 ) & 0xFF, (long long)newsd[37] >> 20, (long long)newsd[37] & 0xFF );
   goto iohtmlFamNewsL1;
  }
  else if( (long long)newsd[2] == CMD_NEWS_ATTACK_FAILED )
  {
   ...
   if ( (long long)newsd[30] > 0 )
   {
    if (b != 0)
     httpString( cnt, ", " );
    httpPrintf( cnt, "%lld Population", (long long)newsd[30]);
   }
...
{
	httpPrintf( cnt, "<tr><td>Interest</td><td align=\"right\" id=\"energyinterest\">+%lld</td></tr>", (long long)(maind.infos[INFOS_ENERGY_INTEREST]) );
}
...
if (cmdRace[maind.raceid].special & CMD_RACE_SPECIAL_WOOKIEE)
{
	httpPrintf( cnt, "<tr><td>Mineral interest</td><td align=\"right\" id=\"mineralinterest\">+%lld</td></tr>", (long long)(maind.infos[INFOS_MINERAL_INTEREST]) );
}
...
if (cmdRace[maind.raceid].special & CMD_RACE_SPECIAL_WOOKIEE)
{
	httpPrintf( cnt, "<tr><td>Crystal interest</td><td align=\"right\" id=\"crystalinterest\">+%lld</td></tr>", (long long)(maind.infos[INFOS_CRYSTAL_INTEREST]) );
}
...
if (cmdRace[maind.raceid].special & CMD_RACE_SPECIAL_WOOKIEE)
{
	httpPrintf( cnt, "<tr><td>Ectrolium interest</td><td align=\"right\" id=\"ectroliuminterest\">+%lld</td></tr>", (long long)(maind.infos[INFOS_ECTROLIUM_INTEREST]) );
}
...
for( a = 0 ; a < CMD_UNIT_NUMUSED ; a++ ) {
 ...
 for( a = 0 ; a < CMD_UNIT_NUMUSED ; a++ )
 {
  ...
  httpPrintf( cnt, "</td><td valign=\"top\" align=\"center\">%lld</td><td valign=\"top\"><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" name=\"unit%dbuild\" size=\"12\"></td></tr>", (long long)maind.totalunit[a], a );
 }
...
httpString( cnt, "</select></td></tr><tr><td align=\"right\">Price :</td><td><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"3\" name=\"price\" size=\"10\"></td></tr><tr><td>Quantity :</td><td><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" name=\"quantity\" size=\"10\"></td></tr></table><input type=\"submit\" value=\"Execute\"></form>" );
...
if( sort < 20 && (( b = dbUserPlanetListIndicesSorted( id, &buffer, sort ) ) <= 0 )) {
	httpString( cnt, "Error while retrieving planets list" );
	iohtmlBodyEnd( cnt );
	return;
}

if(sort >= 20 && (( b = dbUserPlanetListIndicesSortedBonus( id, &buffer, sort ) ) <= 0 )) {
	httpString( cnt, "Error while retrieving planets list" );
	iohtmlBodyEnd( cnt );
	return;
}
...
.dropdown:hover .dropbtn {background-color: #3e8e41;}\
 ...
 httpPrintf(cnt, "<div class=\"dropdown\">");
 httpPrintf(cnt, "<button class=\"dropbtn\" disabled>Bonus</button>");
 httpPrintf(cnt, "<div class=\"dropdown-content\">");
 httpPrintf(cnt, "<a href=\"%s&sort=20\">Solar</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "<a href=\"%s&sort=21\">Mineral</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "<a href=\"%s&sort=22\">Crystal</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "<a href=\"%s&sort=23\">Ectrolium</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "<a href=\"%s&sort=24\">Fission</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "<a href=\"%s&sort=25\">Ancient</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "<a href=\"%s&sort=26\">All</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "<a href=\"%s&sort=27\">None</a>", URLAppend( cnt, "planets" ));
 httpPrintf(cnt, "</div></div>");
...
httpPrintf( cnt, "</td><td>%d</td><td>%d", planetd.size, d );
  ...
  else if (!planetd.special[1] && planetd.special[0] == CMD_BONUS_ANCIENTCIV)
		httpPrintf( cnt, " <img src=\"files?type=image&amp;name=map/pyr.jpeg\" alt=\"%s\" title=\"%s\">", cmdBonusName[planetd.special[0]], cmdBonusName[planetd.special[0]]);
...
//Total
 ...
 httpString(cnt, "</table>");   
 httpString( cnt, "<br><div align=\"right\"><input type=\"submit\" name =\"massbuild\" value=\"Build on selected planets\"></div><br>" );
 
 if( ( dbEmpireGetInfo( maind.empire, &empired ) >= 0 ) && ( empired.numplayers >= 2 ) )
  {
   httpString( cnt, "<br><div align=\"right\"><input type=\"submit\" name=\"massoffer\" value=\"Mass offer\"> " );
  ...
  httpString( cnt, "<br><div align=\"right\"><input type=\"submit\" name=\"massraze\" value=\"Mass Raze\" onclick=\"return confirm('Are you sure?')\"> " );
  httpString( cnt, "<select name=\"razeType\">" );
  for( a = 0 ; a < CMD_BLDG_NUMUSED ; a++ )  
	httpPrintf( cnt, "<option value=\"%d\" >%s", a, cmdBuildingName[a] );
	httpString( cnt, "</select> " );
...
if( ( id = iohtmlIdentify( cnt, 2 ) ) >= 0 ) {
 ...
 if( empired.name[0] )
  iohtmlBodyInit( cnt, "%s %s #%d ( <a href=\"%s&amp;id=%d\">%d,%d</a> )", empired.name, warTag ? "<font color=\"#FF2020\">@war</font>" : "" , curfam, URLAppend( cnt, "system" ), empired.homeid, empired.homepos & 0xFFFF, empired.homepos >> 16 );
 else
  iohtmlBodyInit( cnt, "Empire #%d %s ( <a href=\"%s&amp;id=%d\">%d,%d</a> )", curfam, warTag ? "<font color=\"#FF2020\">@war</font>" : "", URLAppend( cnt, "system" ), empired.homeid, empired.homepos & 0xFFFF, empired.homepos >> 16 );
...
if( empired.picture > 0 ) {
	...
	httpPrintf( cnt, "<br><img src=\"%s\" alt=\"Empire %d Picture\"><br>", fname, curfam );
}
...
empired.numplayers = b;
 ...
 for( a = first ; a != -1 ; )
 {
  ...
  if( ( id >= 0 ) && ( user ) && ( ( curfam == maind.empire ) || (nAlly == CMD_RELATION_ALLY) || ( ( (cnt->session)->dbuser ) && ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) ) )
  {
   ...
   else if( ( empired.politics[CMD_POLITICS_LEADER] == id ) || ( empired.politics[CMD_POLITICS_COMMINISTER] == id ) || ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) )
   {
...
if( ( id >= 0 ) && ( user ) && ( ( curfam == maind.empire ) || ( ( (cnt->session)->dbuser ) && ( ((cnt->session)->dbuser)->level >= LEVEL_MODERATOR ) ) ) ) {
	if( ( empired.politics[CMD_POLITICS_LEADER] == id ) || ( empired.politics[CMD_POLITICS_DEVMINISTER] == id ) || ( ((cnt->session)->dbuser)->level >= LEVEL_MODERATOR ) ) {
...
if( ( empired.politics[CMD_POLITICS_DEVMINISTER] == id ) || ( empired.politics[CMD_POLITICS_LEADER] == id ) ) {
...
iohtmlBodyInit( cnt, "Vote %s", evote ? " for Empire Prime Minister" : ( "Unavalible" ) );
...
if( evote ) {
	...
	if( empired.politics[CMD_POLITICS_LEADER] == -1 ) {
		httpString( cnt, "<b>Your empire didn't elect a Prime Minister yet</b>" );
	} else if( empired.politics[CMD_POLITICS_LEADER] == id ) {
		httpString( cnt, "<b>You are the Prime Minister</b>" );
	} else {
		if( dbUserMainRetrieve( empired.politics[CMD_POLITICS_LEADER], &main2d ) < 0 ) {
			httpString( cnt, "Error while retrieving Prime Minister's main data" );
		} else {
			httpPrintf( cnt, "<b>The empire Prime Minister is %s</b>", main2d.faction );
		}
	}
...
if( ( dbEmpireGetInfo( curfam, &empired ) ) < 0 ) {
 ...
 if( ( b = dbEmpireRelsList( curfam, &rel ) ) >= 0 )
 {
  ...
  for( a = 0 ; a < b ; a += 6 )
  {
   if( (rel[a+3] & 1) && (rel[a + 1] !=  CMD_RELATION_NAP) ) // relation from famtaget
    continue;
   if( rel[a+1] == CMD_RELATION_ALLY )
	...
	if (rel[a+5] == 0 ){ 
		if (rel[a+4] == 7)
			httpPrintf( cnt, "You have an infinite <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> <br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
		else if (rel[a+4] == 8)
			httpPrintf( cnt, "You have sent a cancellation of an infinite <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> <br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
		else if (rel[a+4] == 5)
			httpPrintf( cnt, "You are offering an infinite <font color=\"#3399FF\">non-aggression pact</font> to <a href=\"%s&amp;id=%d\">empire #%d</a> <br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
	}
	else if(rel[a+5] != 0){
		if (rel[a+4] == 3)
			httpPrintf( cnt, "You have a %d-hour <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> <br><br>",rel[a+5], URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );	
		else if (rel[a+4] == 4)
			httpPrintf( cnt, "Your <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> will be cancelled in %d weeks<br><br>", \
		URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], (rel[a+5]*(3600/ticks.speed) -(ticks.number - rel[a])));	
		//URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], (rel[a+5] -(ticks.number - rel[a])));	
		else if (rel[a+4] == 1)
			httpPrintf( cnt, "You are offering a %d-hour <font color=\"#3399FF\">non-aggression pact</font> to <a href=\"%s&amp;id=%d\">empire #%d</a> <br><br>",rel[a+5], URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );		
	}
  }
   else if( rel[a+1] == CMD_RELATION_WAR )
	httpPrintf( cnt, "You declared <font color=\"#FF0000\">war</font> to <a href=\"%s&amp;id=%d\">empire #%d</a>, for another %d weeks.<br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], (rel[a] + (int)GetSetting( "End Wars" )->num_value) - ticks.number );
  }
  for( a = 0 ; a < b ; a += 6 )
  {
   if( !( rel[a+3] & 1 ) && rel[a + 1] !=  CMD_RELATION_NAP  ) //relation to famtarget
    continue;
   if( rel[a+1] == CMD_RELATION_ALLY )
    httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> offered an <font color=\"#80FF80\">alliance</font><br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
   else if( rel[a+1] == CMD_RELATION_NAP ){
	if (rel[a+5] == 0){
		if ( rel[a+4] == 6 )
			httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> offered an infinite <font color=\"#3399FF\">non-aggression pact</font><br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
		else if(rel[a+4] == 9)
			httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> sent an infinite <font color=\"#3399FF\">non-aggression pact</font> cancellation offer<br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
	}
	else if(rel[a+5] != 0){
		if (rel[a+4] == 2)
			httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> offered an %d-hour <font color=\"#3399FF\">non-aggression pact</font><br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], rel[a+5] );	
	}
   }
   else if( rel[a+1] == CMD_RELATION_WAR )
    httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> declared <font color=\"#FF0000\">war</font>! For another %d weeks.<br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], (rel[a] + (int)GetSetting( "End Wars" )->num_value) - ticks.number  );
  }
  httpString( cnt, "<br>" );
  for( a = 0 ; a < b ; a += 6 )
  {
  ...
  if( !( rel[c+3] & 1 ) || ( rel[c+1] != CMD_RELATION_NAP ) || ( rel[a+2] != rel[c+2] ) ) 
     continue;
	if( ( rel[a+5] == rel[c+5] && rel[a+4] == 3 && rel[c+4] == 3 ) || (rel[a+4] == 7 && rel[c+4] == 7 ) ) 	 //conditions for a timed NAP
...
void iohtmlFunc_ministers( ReplyDataPtr cnt ) {
	int a, b, c, id, curfam, sid, status, relfam, reltype, politics, napTime, napStateTransition, relnum = 0;
	int *rel;
	float tax;
	char *empirestring, *fnamestring, *sidstring, *statusstring, *fampassstring; 
	char *relfamstring, *reltypestring, *filename, *taxstring, *napTimeString, *napStateTransitionString, *relNumString;
...
napTimeString = iohtmlVarsFind( cnt, "napTime" );
napStateTransitionString = iohtmlVarsFind( cnt, "napStateTransition" );
reltypestring = iohtmlVarsFind( cnt, "reltype" );
relNumString = iohtmlVarsFind( cnt, "relnum" );
...
//printf("test strings napTimeString %s napStateTransitionString %s reltypestring %s /n", napTimeString, napStateTransitionString, reltypestring);
...
if( ( ( empired.politics[CMD_POLITICS_WARMINISTER] == id ) || ( ( empired.politics[CMD_POLITICS_WARMINISTER] == -1 ) && ( empired.politics[CMD_POLITICS_LEADER] == id ) ) ) || ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) {
	if( reltypestring ) {
		...
		if(relNumString){
			if( sscanf( relNumString, "%d", &relnum ) != 1 )
				goto iohttpFunc_ministersL0;
		}

		if( !( relfamstring ) && relNumString) {

			if( (error = cmdExecDelRelation( curfam, relnum )) < 0 ) {
				if( cmdErrorString )
					httpPrintf( cnt, "<i>%s</i><br><br>", cmdErrorString );
				else
					httpPrintf( cnt, "<i>Error %d while cancelling relation</i><br><br>",error );
			} else {
		...
		if( reltype == CMD_RELATION_NAP && napTimeString && napStateTransitionString){
			sscanf( napTimeString, "%d", &napTime );
			if (sscanf( napStateTransitionString, "%d", &napStateTransition ) <0)
				goto iohttpFunc_ministersL0;
		}

		if(napStateTransition != NAP_TRANSITION_FAM1_CANCEL){
			if (napTime == 0) napStateTransition = NAP_TRANSITION_FAM1_INFINITE;
			else napStateTransition = NAP_TRANSITION_FAM1_TIMED;
		}

		if( (error = cmdExecAddRelation( curfam, reltype, relfam, napTime, napStateTransition )) < 0 ) {
			if( cmdErrorString )
				httpPrintf( cnt, "<i>%s</i><br><br>", cmdErrorString );
			else
				httpPrintf( cnt, "<i>Error %d while adding relation</i><br><br>", error );
		} else {
...
httpString( cnt, "<table>" );
if( ( empired.politics[CMD_POLITICS_LEADER] == id ) || ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) {
if (maind.benefactor[4] != 1){ //if the user is not muted
...
if( empired.numplayers > 1 ) {
	...
	httpString( cnt, "<tr><td><i>Minister of Communication can edit and delete posts in the forum.<br>Factions marked rebellious aren't allowed to read the empire forum, and can lose their home planet.</i></td></tr>" );
...
httpString( cnt, "<tr><td><i>Leave the field blank to let anyone join, if you forget it... just change it! =)</i></td></tr>" );
...
if( ( b = dbEmpireRelsList( curfam, &rel ) ) >= 0 ) {
	...
	printf("rel6 %d \n", rel[a +5] );*/
		...
		if( rel[a+1] == CMD_RELATION_ALLY ) {
				...
				httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&relnum=%d\">Cancel</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_ALLY,a/6 );
		} 
		else if( rel[a+1] == CMD_RELATION_NAP ) {

				if (rel[a+5] == 0 ){ 
					if (rel[a+4] == 7){
						httpPrintf( cnt, "You have an infinite <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> <br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&napStateTransition=%d&relnum=%d\">Cancel</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_NAP, NAP_TRANSITION_FAM1_CANCEL,a/6 );
					}
					else if (rel[a+4] == 8)
						httpPrintf( cnt, "You have sent a cancellation of an infinite <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> <br><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
					else if (rel[a+4] == 5){
						httpPrintf( cnt, "You are offering an infinite <font color=\"#3399FF\">non-aggression pact</font> to <a href=\"%s&amp;id=%d\">empire #%d</a> <br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&napStateTransition=%d&relnum=%d\">Cancel</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_NAP, NAP_TRANSITION_FAM1_CANCEL, a/6 );
					}
				}
				else if(rel[a+5] != 0){
					if (rel[a+4] == 3){
						httpPrintf( cnt, "You have a %d-hour <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> <br>",rel[a+5], URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );	
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&napStateTransition=%d&relnum=%d\">Cancel</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_NAP, NAP_TRANSITION_FAM1_CANCEL,a/6 );
					}
					else if (rel[a+4] == 4)
						httpPrintf( cnt, "Your <font color=\"#3399FF\">non-aggression pact</font> with <a href=\"%s&amp;id=%d\">empire #%d</a> will be cancelled in %d weeks<br><br>",\
					URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], (rel[a+5]*(3600/ticks.speed) -(ticks.number - rel[a])),rel[a+5] );	
					//URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], (rel[a+5] -(ticks.number - rel[a])),rel[a+5] );	
					else if (rel[a+4] == 1){
						httpPrintf( cnt, "You are offering a %d-hour <font color=\"#3399FF\">non-aggression pact</font> to <a href=\"%s&amp;id=%d\">empire #%d</a> <br>",rel[a+5], URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );		
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&napStateTransition=%d&relnum=%d\">Cancel</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_NAP, NAP_TRANSITION_FAM1_CANCEL, a/6 );
					}
				}

				//httpPrintf( cnt, "You are offering a %d-hour non-aggression pact to <a href=\"%s&amp;id=%d\">empire #%d</a> - ", rel[a+5],URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
				
		} else if( rel[a+1] == CMD_RELATION_WAR ) {
				httpPrintf( cnt, "You declared war on <a href=\"%s&amp;id=%d\">empire #%d</a> - ", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
				httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&relnum=%d\">Cancel</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_WAR, a/6 );
		}
	}
	for( a = 0 ; a < b ; a += 6 ) {
		...
		if( rel[a+1] == CMD_RELATION_ALLY ) {
			httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> offered an alliance",URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
			if( cmdExecFindRelation( curfam, rel[a+2], 0, 0 ) != CMD_RELATION_ALLY ) {
				httpPrintf( cnt, " - <a href=\"%s&amp;id=%d&relfam=%d&reltype=%d&relnum=%d\">accept</a><br>", URLAppend( cnt, "ministers" ), curfam, rel[a+2], CMD_RELATION_ALLY, a/6 );
			}
			httpString( cnt, "<br>" );
		} 
		else if( rel[a+1] == CMD_RELATION_NAP) {
				...
				if (rel[a+5] == 0){
					if ( rel[a+4] == 6 ){
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> offered an infinite <font color=\"#3399FF\">non-aggression pact</font><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&napStateTransition=%d&relfam=%d&napTime=%d&relnum=%d\">accept</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_NAP, NAP_TRANSITION_FAM1_INFINITE,rel[a+2],rel[a+5],a/6 );
					}
					else if(rel[a+4] == 9){
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> sent an infinite <font color=\"#3399FF\">non-aggression pact</font> cancellation offer<br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2] );
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&napStateTransition=%d&relnum=%d\"> cancel the nap</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_NAP, NAP_TRANSITION_FAM1_CANCEL, a/6  );
					}
				}
				else if(rel[a+5] != 0){
					if (rel[a+4] == 2){
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> offered an %d-hour <font color=\"#3399FF\">non-aggression pact</font><br>", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2], rel[a+5] );	
						httpPrintf( cnt, "<a href=\"%s&amp;id=%d&reltype=%d&napStateTransition=%d&relfam=%d&napTime=%d&relnum=%d\">accept</a><br>", URLAppend( cnt, "ministers" ), curfam, CMD_RELATION_NAP, NAP_TRANSITION_FAM1_TIMED,rel[a+2],rel[a+5], a/6 );
					}
				}
			
			//httpPrintf( cnt, "<a href=\"%s&amp;id=%d\">Empire #%d</a> offered a %d-hour non-aggression pact", URLAppend( cnt, "empire" ), rel[a+2], rel[a+2],rel[a+5] );
...
httpPrintf( cnt, "<tr><td><form action=\"%s\" method=\"POST\">Send NAP Offer, set 0 hr for an infinite NAP</td></tr>", URLAppend( cnt, "ministers" ) );
httpPrintf( cnt, "<input type=\"hidden\" name=\"id\" value=\"%d\">", curfam );
httpString( cnt, "<tr><td>Empire: <input type=\"text\" name=\"relfam\" size=\"8\"><br><br> Nr. of hours: <input type=\"number\" name=\"napTime\" size=\"8\" min=\"0\" max=\"100\">\
<br><input type=\"hidden\" name=\"reltype\" value=\"1\"><input type=\"hidden\" name=\"napStateTransition\" value=\"0\"> \
<input type=\"submit\" value=\"Send\"></form><br><br><br></td></tr>" );
...
if( ( empired.politics[CMD_POLITICS_LEADER] == id ) || ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) {
	...
	httpPrintf( cnt, "<tr><td><input type=\"hidden\" name=\"id\" value=\"%d\"><textarea name=\"hqmes\" style=\"height:200px\" wrap=\"soft\" rows=\"4\" cols=\"64\">%s</textarea></td></tr>", curfam, msg[1] );
	httpString( cnt, "<tr><td><input type=\"submit\" value=\"Change\"></form><br><br><br></td></tr>" );
}/*
if( ( empired.politics[CMD_POLITICS_COMMINISTER] == id ) || ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) {
...
if( ( ( empired.politics[CMD_POLITICS_WARMINISTER] == id ) || ( ( empired.politics[CMD_POLITICS_WARMINISTER] == -1 ) && ( empired.politics[CMD_POLITICS_LEADER] == id ) ) ) || ( (cnt->session)->dbuser->level >= LEVEL_MODERATOR ) ) {
	...
	httpPrintf( cnt, "<tr><td><input type=\"hidden\" name=\"id\" value=\"%d\"><textarea name=\"relsmes\"style=\"height:200px\" wrap=\"soft\" rows=\"4\" cols=\"64\">%s</textarea></td></tr>", curfam, msg[1] );
	httpString( cnt, "<tr><td><input type=\"submit\" value=\"Change\"></form><br><br><br></td></tr>" );
}
httpString( cnt, "</table>" );
...
{
 int a, b, c, d, numpl, id, x, y, i, explcol, cid, plistid;
 int *buffer;
 dbUserMainDef maind;
...
if( iohtmlVarsFind( cnt, "request" ) ) {
 ...
 for( y = basey ; y < endy ; y++ )
 {
  ...
  for( x = basex ; x < endx ; x++, i++ )
  {
   ...
   {
	dbMapRetrieveSystem( (( mapp[i] & 0xFFFF ) - 1), &systemd );
    a = '0' + ( ( mapp[i] >> 16 ) & 0xF );
    b = '5';
    if( !( mapp[i] >> 24 ) )
     b = '0' + ( rand() % 5 );
	...
	plistid = CheckPlistUserId(id);
		
	if (plistid > -1 && plist[plistid].planetlistID[( mapp[i] & 0xFFFF ) - 1] != 0){ //system belongs to the user list
		httpPrintf( cnt, "<img style=\"background-image:url(files?type=image&amp;name=map/m%c%c.gif);\" src=\"files?type=image&amp;name=map/select.gif\" title=\"%d,%d planets: %d\"></div></a>", a ,b, x, y, systemd.numplanets);
	}
	else
		httpPrintf( cnt, "<img border=\"0\" src=\"files?type=image&amp;name=map/m%c%c.gif\" title=\"%d,%d planets: %d\"></a>", a, b, x, y, systemd.numplanets );
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 httpPrintf( cnt, "<a href=\"%s&amp;request=true&amp;sectorzoom=%d,%d\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">%d,%d</a> ( click to zoom there ).<br><br>", URLAppend( cnt, "map" ), a * IOHTTP_MAPPICK_DIVIDE, b * IOHTTP_MAPPICK_DIVIDE, a, b );
 ...
 httpPrintf( cnt, "<td colspan=\"3\" rowspan=\"3\"><img src=\"files?type=image&amp;name=galaxies/galaxyr%d.png\" style=\"width:240px; height:240px;\" alt=\"Planets\" usemap=\"#systemmap\">", ticks.round/*, dbMapBInfoStatic[MAP_SIZEX] *IOHTTP_MAPPICK_DIVIDE, dbMapBInfoStatic[MAP_SIZEY] *IOHTTP_MAPPICK_DIVIDE*/ );
...
for( i = 0; i < dbMapBInfoStatic[MAP_SYSTEMS]; i++ ) {
	...
	httpPrintf( cnt, "<area shape=\"circle\" coords=\"%d,%d,8\" alt=\"system%d\" href=\"%s&amp;request=true&amp;sectorzoom=%d,%d\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">", px, py, i, URLAppend( cnt, "map" ), px, py );
...
{
 int a, b, id, i, j, plistID;
 dbUserMainDef maind;
 static char *advopt[8] = { ">Unused", ">Your planets", ">Your portals", ">Your empire", ">Planets of faction :", ">Planets of empire :", ">Unexplored planets" };
 ...
 extern StructPlist *plist;
 char *temp;
 
if (plist == null){
  plist = malloc(dbMapBInfoStatic[MAP_CAPACITY] * sizeof(StructPlist));
  for (i=0; i < dbMapBInfoStatic[MAP_CAPACITY]; i++)
	{
		plist[i].id = -1;
	}
 }
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 httpPrintf( cnt, "<a href=\"%s&amp;e0=1&amp;c0=0&amp;setdefault=1&amp;request=true\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">Your planets in green</a><br>", URLAppend( cnt, "map" ) );
 httpPrintf( cnt, "<a href=\"%s&amp;e0=1&amp;c0=1&amp;e1=2&amp;c1=0&amp;setdefault=1&amp;request=true\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">Your planets in blue and portals in green</a><br>", URLAppend( cnt, "map" ) );
 httpPrintf( cnt, "<a href=\"%s&amp;e0=3&amp;c0=2&amp;e1=1&amp;c1=1&amp;e2=2&amp;c2=0&amp;setdefault=1&amp;request=true\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">Your empire in red, your planets in blue and portals in green</a><br><br></table>", URLAppend( cnt, "map" ) );


 plistID = FindPlistNrFromUserId(id, plist);
 
 if (plistID == -1)
	 goto end;
 
 httpPrintf( cnt, "<form action=\"\" method=\"POST\">" );
 httpPrintf( cnt, "<b>Input planet coordinates to show on the map</b><br><br>" );
 httpPrintf( cnt, "<textarea rows=\"10\" cols=\"45\" id =\"plist\" name=\"planet_list\">%s</textarea><br>", plist[plistID].planet_list );
 httpPrintf( cnt, "<input type=\"submit\" value=\"Save\" >&nbsp;&nbsp;&nbsp;&nbsp;" );
 httpPrintf( cnt, "<input type=\"submit\" name =\"reset\" value=\"Reset\" >" );
 
 if (iohtmlVarsFind( cnt, "planet_list" ) != NULL && iohtmlVarsFind( cnt, "reset") == NULL ){
	temp = strdup(iohtmlVarsFind( cnt, "planet_list" ));
	plist[plistID].planet_list = GenerateList(temp , plist[plistID].planetlistID, cnt);
	temp = (char*) realloc(temp, 2*(strlen(plist[plistID].planet_list)+1)*sizeof(char));
	temp[0]='\0';
	for (i = 0, j = 0; j < strlen(plist[plistID].planet_list); ++j){
		if (plist[plistID].planet_list[j] == '\n'){
			temp[i++] = '\\';
			temp[i++] = 'n';
		}	
		else
		{
			temp[i++] = plist[plistID].planet_list[j];
		}
	}
	temp[i]='\0';
	httpPrintf( cnt, "<script>document.getElementById('plist').value = \"%s\"</script>", temp);
	free(temp);
  }
  
  if (iohtmlVarsFind( cnt, "reset") != NULL){
    httpPrintf( cnt, "<script>document.getElementById('plist').value = \"\" </script>");
	plist[plistID].planet_list[0] = '\0';
	plist[plistID].planet_list = (char*) realloc(plist[plistID].planet_list, 1*sizeof(char));
	
	for (i=0; i < dbMapBInfoStatic[MAP_SYSTEMS]; ++i)
		plist[plistID].planetlistID[i] = 0;
  }	
...
int dbUserObservesSurveys_CompareDsc(const void * A, const void * B){
	int psize_A, psize_B;
	float str_A, str_B;
	dbMainPlanetDef planetd_A;
	dbMainPlanetDef planetd_B;
	psize_A = *(int*)A;
	psize_B = *(int*)B;
	str_A = *((float*)A+1);
	str_B = *((float*)B+1);
	...
	if (str_A == 1.0)
	{
		if (!dbMapRetrievePlanet( psize_A, &planetd_A ))
			return 0;
		psize_A = planetd_A.size;
	}
	else
		psize_A = 0.0;
	...
	if (str_B == 1.0)
	{
		if (!dbMapRetrievePlanet( psize_B, &planetd_B ))
			return 0;
		psize_B = planetd_B.size;
	}
	else
		psize_B = 0.0;
	
	return psize_A - psize_B;
}

int dbUserObservesSurveys_CompareAsc(const void * A, const void * B){
	int psize_A, psize_B;
	float str_A, str_B;
	dbMainPlanetDef planetd_A;
	dbMainPlanetDef planetd_B;
	...
	return psize_B - psize_A;
}

char * getsystems(void * planet_list, int num_planets){
	dbMainPlanetDef planetd;
	char * systems;
	int i;
	
	systems = malloc(dbMapBInfoStatic[MAP_SYSTEMS]*sizeof(char));
	
	for (i = 0 ; i < dbMapBInfoStatic[MAP_SYSTEMS]; i++)
		systems[i] = 0;
	
	for (i=0; i < num_planets; i++){
		dbMapRetrievePlanet( ((int*)planet_list)[i*2], &planetd );
		systems[planetd.system] = 1;
	}
	return systems;
}

void populateObserveList(char * systems, int plistID){
	int i, j, x, y, x_digits, y_digits;
	dbMainSystemDef systemd;
	char *generated_list, *list_element;
	
	i = dbMapBInfoStatic[MAP_SIZEX];
	for (x_digits = 1; (i /= 10) > 1; x_digits++);
	
	i = dbMapBInfoStatic[MAP_SIZEY];
	for (y_digits = 1; (i /= 10) > 1; y_digits++);
	
	list_element  = malloc((x_digits + y_digits + 10) *sizeof(char));
	  
	generated_list = malloc(dbMapBInfoStatic[MAP_SYSTEMS] * (x_digits + y_digits + 10) *sizeof(char));
	generated_list[0] = '\0';
	
	for (i=0, j=0; i < dbMapBInfoStatic[MAP_SYSTEMS]; i++){
		if (systems[i] != 0){
			dbMapRetrieveSystem( i, &systemd );
			x = systemd.position & 0xFFFF;
			y = systemd.position >> 16;
			sprintf (list_element, "%d,%d\n", x, y );
			strcpy(generated_list + j,list_element);
			j += strlen(list_element);
		}
	}
	generated_list[j] = '\0';
	plist[plistID].planet_list = generated_list;
	free(list_element);
}

void iohtmlFunc_ObservesSurveys( ReplyDataPtr cnt)
{
 int empire_id, id, planet_num, i = 0, planet_id, sort, hide_taken;
 dbUserMainDef maind,main2d;
 void * planet_list;
 float obs_factor;
 dbMainPlanetDef planetd;
 ...
 if( !( iohtmlHeader( cnt, id, &maind ) ) )
  return;
 iohtmlBodyInit( cnt, "Scouting" );
 
 empire_id = maind.empire;
 planet_num = dbUserObservesSurveys_size(empire_id);
 
 sortstring = iohtmlVarsFind( cnt, "sort" );
 if( !( sortstring ) || ( sscanf( sortstring, "%d", &sort ) <= 0 ) )
  sort = 0;

 sortstring = iohtmlVarsFind( cnt, "hide_taken" );
 if( !( sortstring ) || ( sscanf( sortstring, "%d", &hide_taken ) <= 0 ) )
  hide_taken = 0;
 
 if ( !planet_num )
	httpString(cnt,  "Your empire hasn't done any observes or surveys yet.");
 else{ 
	planet_list = malloc(2 * (planet_num) * sizeof(int));
	if (!dbUserObservesSurveys_read(empire_id, planet_list))
		return;
	httpPrintf( cnt, "<form action=\"\" method=\"POST\">" );
	httpPrintf( cnt, "<p style=\"text-align:left\">Load this list into map gen <input type=\"submit\" name=\"generate_list\" value=\"Load\"></style>" );
	httpPrintf( cnt, "" );
	
	if (iohtmlVarsFind( cnt, "generate_list" ))
	{
		extern StructPlist *plist;
		int plistID;
		if (plist == null){
		...
		systems = getsystems(planet_list, planet_num);
		plistID = FindPlistNrFromUserId(id, plist);
		if (plistID == -1)
			goto end;	
		plist[plistID].planetlistID = systems;
		populateObserveList(systems,plistID);
		httpPrintf( cnt, " - Done!" );
	}
	
	if ( sort == 1 ) //sort from low to high;
	{
		qsort(planet_list, planet_num, 2 * sizeof(int), dbUserObservesSurveys_CompareAsc);
	}
	else if( sort == 2 )
	{
		qsort(planet_list, planet_num, 2 * sizeof(int), dbUserObservesSurveys_CompareDsc);
	}

	httpString(cnt, "</form><p style=\"text-align:left\"><font color=\"#FF2020\">!</font> - The planet wasn't fully observed/surveyed, some important info might be missing.<br></style>");
	httpString( cnt, "<table style=\"width:100%\">");
	httpString( cnt, "<tr><td><b><u>Planet</td>");
	
	if (sort == 0 || sort == 2 )
		httpPrintf( cnt, "<td><b><u><a href=\"/%s&sort=1&hide_taken=%d\">Size</a></td>", URLAppend( cnt, "observessurveys" ),hide_taken);
	else 
		httpPrintf( cnt, "<td><b><u><a href=\"/%s&sort=2&hide_taken=%d\">Size</a></td>", URLAppend( cnt, "observessurveys" ),hide_taken);
	
	httpString(cnt, "<td><b><u>Bonus</td>");
	
	if (hide_taken == 1 )
		httpPrintf( cnt, "<td><b><u><a href=\"/%s&hide_taken=0&sort=%d\">Status</a></td></tr>", URLAppend( cnt, "observessurveys" ), sort);
	else 
		httpPrintf( cnt, "<td><b><u><a href=\"/%s&hide_taken=1&sort=%d\">Status</a></td></tr>", URLAppend( cnt, "observessurveys" ), sort);
	
	 httpPrintf( cnt, "<form action=\"%s\" method=\"POST\" id=\"mass_explore\"><p style=\"text-align:right\"><input type=\"submit\" value=\"Explore selected planets\"></style><br>", URLAppend( cnt, "massexplore" ) );

	for (i=0; i < 2*(planet_num); i += 2 ){
		planet_id = *((int*)planet_list+i);
		obs_factor = *((float*)planet_list+i+1);
		if (!dbMapRetrievePlanet( planet_id, &planetd ))
			goto end;
		...
		if (planetd.owner == -1 && planetd.nuked == -1){
			...
			httpPrintf( cnt, "</td><td align=\"center\"><input type=\"checkbox\" name=\"m%d\"></td></tr>", planet_id );
			}
		}
		else if(planetd.nuked != -1)
			httpPrintf( cnt, "<td><font color=\"#FF2020\">nuked and unexplorable</font></td></tr>");
		else if (planetd.owner == id)
			httpPrintf( cnt, "<td><font color=\"#FFFF00\">allready yours</font></td></tr>");
		else {
			 if( dbUserMainRetrieve( planetd.owner, &main2d ) < 0 )
				goto end;
			httpPrintf( cnt, "<td><font color=\"#FF2020\">taken by %s</font></td></tr>",main2d.faction);
		}
		

		httpPrintf( cnt, "</td></tr>");
	}
	httpPrintf( cnt, "</form></table>");
...
void iohtmlFunc_massexplore(ReplyDataPtr cnt){
 ...
 int id, planet_num, empire_id, i, planet_id, net = 0, cost = 0, count = 0, explore;
 char buildname[16];
 void * planet_list;
 dbUserMainDef maind;
 dbMainPlanetDef planetd;
 ...
 if( !( iohtmlHeader( cnt, id, &maind ) ) )
  return;
 iohtmlBodyInit( cnt, "Mass explore" );
  
 empire_id = maind.empire;
 planet_num = dbUserObservesSurveys_size(empire_id);
 planet_list = malloc(2 * (planet_num) * sizeof(int));
 dbUserObservesSurveys_read(empire_id, planet_list);

 for (i=0; i < 2*(planet_num); i += 2 ){
	planet_id = *((int*)planet_list+i);
	sprintf( buildname, "m%d", planet_id );
	if( !( iohtmlVarsFind( cnt, buildname ) ) )
     continue;
	
	if( ( dbMapRetrievePlanet( planet_id, &planetd ) < 0 ) ) {
	...
	if ( planetd.owner == -1 ) {
		...
		int test;
		if( ( test = cmdExecExplore( id, planet_id, &explore ) ) < 0 ) {			
			  if( dbUserFleetList( id, &fleetd )  <= 0 ) {
			...
			if ( (maind.readiness[0] >> 16) < 0)
				httpString(cnt, "You run out of fleet readiness!<br>");
			if ( fleetd[0].unit[CMD_UNIT_EXPLORATION] <= 0 )
				httpString(cnt, "You run out of exploration ships!<br>");
			break;
		} 
		else {
			httpPrintf( cnt, "Exploration ship sent to <a href=\"%s&amp;id=%d\">%d,%d:%d</a> travel time %d week(s).<br>", URLAppend( cnt, "planet" ) , planet_id, ( planetd.position >> 8 ) & 0xFFF, planetd.position >> 20, planetd.position & 0xFF, explore);
			cost += net;
			count++;
		}
	}
 }
 httpPrintf(cnt, "Sent %d exploration ships, this costed %d fr.<br>",count, cost);
...
//parse the list to get coordinates and their sys id
//returns xx,yy list and populates planetlistID table 
char* GenerateList(char * planetlist, char * planetlistID, ReplyDataPtr cnt)
{
	int i,j,x_digits, y_digits ,x,y,state_counter;
	int sysid, sysnr = 1;
	char *generated_list;
	char *list_element;
	char *temp;
	...
	x = -1;
	y = -1;
	for (i=0, j=0; planetlist[i] !='\0'; i++)	{
	
		if (isdigit(planetlist[i]) || planetlist[i] == ',' ){
			
			if (state_counter == -1){ //initialize x and y coords
				x = 0;
			}
			
			if (state_counter == x_digits  && planetlist[i] != ','){  //x coord overflow
				x = -1;
				state_counter = x_digits +y_digits + 1;
			}
			
			//search for x coords
			if (state_counter < x_digits && planetlist[i] != ','){ //find x coord
				x = x*10 + (planetlist[i] - '0');
				++state_counter;
			}
			
			if (state_counter == x_digits +y_digits +1){ //y coord overflow
				++state_counter;
				y = -1;
			}
			
			//search for y coords
			if (state_counter >= x_digits && state_counter < x_digits +y_digits +1 && planetlist[i] != ','){ //find y coords
				y = y*10 + (planetlist[i] - '0');
				++state_counter;
			}
			
			if (state_counter >= x_digits && planetlist[i] == ',' && y < 0 ){ //comma overflow
				x = -1;
				y = -1;
				state_counter = x_digits +y_digits +1;
			}
			
			if (state_counter > -1 && state_counter < x_digits && planetlist[i] == ','){ //find comma
				state_counter = x_digits +1;
				y = 0;
			}
		}
		else {
			...
			if (x >=0 && y >= 0){
				//printf("x: %d, y: %d xd: %d yd: %d counter %d\n",x ,y, x_digits, y_digits, state_counter );	
				sysid = dbMapFindSystem( x, y ); // find systems id from x and y coords
				
				if (planetlistID[sysid] == 0 && sysid > -1){
					planetlistID[sysid] = 1;
					sprintf (list_element, "%d,%d\n", x, y );
					if (sysnr > dbMapBInfoStatic[MAP_SYSTEMS]){
						httpPrintf( cnt, "<br>You've enetered more systems than there are on the map!<br>" );
						goto end;
					}	
					strcpy(generated_list + j,list_element);
					j += strlen(list_element);
	...
	if (x >=0 && y >= 0){
		sysid = dbMapFindSystem( x, y );
		if (planetlistID[sysid] == 0 && sysid > -1){
			if (sysnr > dbMapBInfoStatic[MAP_SYSTEMS]){
				httpPrintf( cnt, "<br>You've enetered more systems than there are on the map!<br>" );
				goto end;
			}	
			// find systems id from x and y coords
			planetlistID[sysid] = 1;
			sprintf (list_element, "%d,%d\n", x, y );
			strcpy(generated_list + j,list_element);
			++sysnr;
	...
	httpPrintf( cnt, "<br>%d unique systems were found present on the map<br>",sysnr-1 );
	end:
	free(list_element);
	generated_list = temp;
...
int FindPlistNrFromUserId(int id, StructPlist *plist)
{
	...
	for (i=0; i < dbMapBInfoStatic[MAP_CAPACITY]; i++)
	{
		...
		else if(plist[i].id == -1){
			plist[i].id = id;
			plist[i].planet_list = strdup("");
			plist[i].planetlistID = malloc((dbMapBInfoStatic[MAP_SYSTEMS])*sizeof(char));
			for (j=0; j < dbMapBInfoStatic[MAP_SYSTEMS]; ++j)
				plist[i].planetlistID[j] = 0;
			return i;
		}
	}
	info("error in FindPlistNrFromUserId");
	return -1;
}
...
if( ( systemd.unexplored > 1 ) && ( systemd.empire == -1 ) )
 httpPrintf( cnt, "<br>There are %d unexplored planets in this system<br><a href=\"%s&system=%d\">Mass explore system</a><br>", systemd.unexplored, URLAppend( cnt, "explore" ), sysid);
 
 if( specopAgentsAllowed( CMD_OPER_OBSERVEPLANET, maind.raceid ) || roundspec_define[ROUNDSPEC_OBSERVEFORALL] == 1 )
	httpPrintf( cnt, "<a href=\"%s&system=%d\">Mass observe system</a><br>", URLAppend( cnt, "observe" ), sysid);
 ...
 for( a = b = ln = 0 ; a < systemd.numplanets ; a++, plnid++ )
 {
  ...
  httpPrintf( cnt, "<a href=\"%s&amp;id=%d\"><img src=\"files?type=image&amp;name=map/p%02d.gif\" border=\"0\" alt=\"Planet #%d\"></a><br>", URLAppend( cnt, "planet" ), plnid, pics[a], plnid );
  ...
  if( planetd.owner == -1 )
  {
	if (planetd.nuked == -1)
	  ...
	  httpPrintf( cnt, "<font color=\"#8090A0\">Planet %d nuked</font>", a );
...
if( ((cnt->session)->dbuser)->level >= LEVEL_ADMINISTRATOR ) {
 httpPrintf(cnt, "<b>planet id: %d nuked: %d</b><br><br>", planetd.id, planetd.nuked);
  httpPrintf(cnt, "<b>arti presense: %d</b><br><br>", artefactPrecense(&planetd));
  httpPrintf(cnt, "<b>Planet bonus: %d %d</b><br><br>", planetd.special[0],planetd.special[1] );
}
...
if( planetd.flags & CMD_PLANET_FLAGS_MEGA ) {
	httpPrintf( cnt, "<img src=\"files?type=image&amp;name=map/p16.gif\" border=\"0\" alt=\"Planet %d\"><br><br>", plnid );
} else {
	srand( planetd.system );
	for( a = 0 ; a < b ; a++ )
		rand();
	httpPrintf( cnt, "<img src=\"files?type=image&amp;name=map/p%02d.gif\" border=\"0\" alt=\"Planet %d\"><br><br>", rand() & 0xF, plnid );
}
 ...
 if( planetd.owner == -1)
 {
  if( planetd.flags & CMD_PLANET_FLAGS_HOME )
   httpPrintf( cnt, "No one owns this planet, but it is part of a home system and unavailable for exploration" );
	...
	httpPrintf( cnt, "This planet suffered a heavy nuclear blast, polution is too high to inhabit it!" );
	else {
  ...
  if( b >= 0 )
   httpPrintf( cnt, "<br><img src=\"files?type=image&amp;name=artefact/%s\" alt=\"%s\" title=\"%s\"> %s<br>", artefactImage[b], artefactName[b], artefactName[b], artefactDescription[b] );//ArtefactTable[b]->image, ArtefactTable[b]->name, ArtefactTable[b]->name, ArtefactTable[b]->description );
		...
		if (planetd.special[0] != CMD_BONUS_ANCIENTCIV)
			...
			httpPrintf( cnt, "<br><img src=\"files?type=image&amp;name=map/ancient.gif\" alt=\"%s\" title=\"%s\"> %s <font color=\"#20FF20\"></font><br>", cmdBonusName[planetd.special[0]], cmdBonusName[planetd.special[0]], cmdBonusName[planetd.special[0]] );
  }
...
*/
 ...
 for( a = 0 ; a < CMD_BLDG_NUMUSED ; a++ )
 {
  ...
  if( resbuild[b] )
   httpPrintf( cnt, "<font color=\"#FF2020\">Tech : +%lld%% costs</font><br>", (long long)resbuild[b] );
  httpPrintf( cnt, "</td><td valign=\"top\" align=\"center\">%lld</td><td valign=\"top\"><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" size =\"10\" name=\"bldg%d\" ></td></tr>", (long long)maind.totalbuilding[a], a );
 }
...
void iohtmlFunc_massoffer( ReplyDataPtr cnt ){
 ...
 for( i = 0 ; i < numpl ; i++ )
 {
  sprintf( buildname, "m%d", buffer[i] );
  if( !( iohtmlVarsFind( cnt, buildname ) ) )
   continue;

  if( ( dbMapRetrievePlanet( buffer[i], &planetd ) < 0 ) )
   continue;

  cmdExecOfferPlanet( id, plgive, planetd.id );
  planetd.surrender = plgive;

 }
 free(buffer);
 iohtmlFunc_planets(cnt);	
}

void iohtmlFunc_massRaze( ReplyDataPtr cnt ){
	int id, b, i, cmd[5], plraze, total = 0, numPlanets = 0, numpl;
	dbUserMainDef maind;
	dbMainPlanetDef planetd;
	dbMainEmpireDef empired;
	char *razeType;
 ...
 if ( dbEmpireGetInfo( maind.empire, &empired ) < 0){
  ...
  razeType = iohtmlVarsFind( cnt, "razeType" ); 
  
  if( razeType )
	sscanf( razeType, "%d", &plraze );
 ...
 for( i = 0 ; i < numpl ; i++ )
 {
  ...
  sprintf( planetName, "m%d", buffer[i] );
  if( !( iohtmlVarsFind( cnt, planetName ) ) )
   continue;

  if( ( dbMapRetrievePlanet( buffer[i], &planetd ) < 0 ) )
   continue;

 cmd[2] = plraze;// building type
 cmd[3] = planetd.building[plraze];// number of buildings
 cmd[4] = planetd.id;//planet id
  ...
  httpPrintf( cnt, "<i>You razed a total of %d %s on %d planets</i><br><br>", total, cmdBuildingName[plraze],numPlanets );
  free(buffer);
  iohtmlBodyEnd( cnt );	
}
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 if (iohtmlVarsFind( cnt, "massraze" )){
	iohtmlFunc_massRaze(cnt);
	return;
 }
 
  if (iohtmlVarsFind( cnt, "massoffer" )){
	iohtmlFunc_massoffer(cnt);
	return;
 }
...
*/
 ...
 httpString( cnt, "<input type=\"checkbox\" name=\"obcheck\"> build between overbuilding penalty from <input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" name=\"oblimitlow\" size=\"4\"> to <input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" name=\"oblimit\" size=\"4\"> %" );
 httpString( cnt, "</td></tr></table>" );
 ...
 for( a = 0 ; a < CMD_BLDG_NUMUSED ; a++ )
 {
  ...
  if( resbuild[b] )
   httpPrintf( cnt, "<font color=\"#FF2020\">Tech : +%lld%% costs</font><br>", (long long)resbuild[b] );
  httpPrintf( cnt, "</td><td valign=\"top\" align=\"center\">%lld</td><td valign=\"top\"><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" size =\"8\" name=\"bldg%d\" ></td></tr>", (long long)maind.totalbuilding[a], a );
 }
...
for( a = b = 0 ; a < CMD_UNIT_NUMUSED ; a++ ) {
 ...
 httpString( cnt, "<tr><td align=\"center\">Approximations of enemy forces to flee battles, 0 - 9999%<tr><td align=\"center\">" );

 httpString( cnt, "<table>" );
 httpPrintf( cnt, "<tr><td align=\"right\" nowrap>Long-range attack</td><td><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"4\" size =\"5\"  name=\"f0\" size=\"4\" value=\"%d\"> %%</td></tr>", maind.config_flee[0] );
 httpPrintf( cnt, "<tr><td align=\"right\" nowrap>Air vs air</td><td><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"4\" size =\"5\"  name=\"f1\" size=\"4\" value=\"%d\"> %%</td></tr>", maind.config_flee[1] );
 httpPrintf( cnt, "<tr><td align=\"right\" nowrap>Ground vs air</td><td><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"4\" size =\"5\" name=\"f2\" size=\"4\" value=\"%d\"> %%</td></tr>", maind.config_flee[2] );
 httpPrintf( cnt, "<tr><td align=\"right\" nowrap>Ground vs ground</td><td><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"4\" size =\"5\" name=\"f3\" size=\"4\" value=\"%d\"> %%</td></tr>", maind.config_flee[3] );
 httpString( cnt, "</table>" );
 ...
 {
  if( fleetd[0].unit[a] )
   httpPrintf( cnt, "<tr><td align=\"right\" id=\"dunt%d\">%d</td><td align=\"right\">%s</td><td><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"12\" size =\"13\" name=\"u%d\" size=\"12\"></td></tr>", a, fleetd[0].unit[a], cmdUnitName[a], a );
 }
...
void iohtmlAttackReport( ReplyDataPtr cnt, int64_t *results, int sats, int pop )
{
 ...
 if (popkilled2>0 && pop){ //pop defence
	 httpPrintf( cnt, "%lld population", popkilled2);
 }
 if( sats )
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 iohtmlAttackReport( cnt, &results[0*CMD_UNIT_FLEET], results[4+8*CMD_UNIT_FLEET+0], 0 );
 if( ( results[3] & 0x10 ) )
 ...
 if( ( results[3] & 0x20 ) )
  ...
  httpString( cnt, "Your fleet didn't encounter much resistance, it seems enemy forces preferred not to directly engage us<br>" );
 iohtmlAttackReport( cnt, &results[2*CMD_UNIT_FLEET], results[4+8*CMD_UNIT_FLEET+1], 0 );
 if( ( results[3] & 0x20 ) )
  goto iohttpFunc_fleetattackL0;
 ...
 if( ( results[3] & 0x40 ) )
  ...
  httpString( cnt, "Your fleet didn't encounter much resistance, it seems enemy forces preferred not to directly engage us<br>" );
 iohtmlAttackReport( cnt, &results[4*CMD_UNIT_FLEET], 0, 0 );
 if( ( results[3] & 0x40 ) )
  goto iohttpFunc_fleetattackL0;
 ...
 if( ( results[3] & 0x80 ) )
  ...
  httpString( cnt, "Your fleet didn't encounter much resistance, it seems enemy forces preferred not to directly engage us<br>" );
 iohtmlAttackReport( cnt, &results[6*CMD_UNIT_FLEET], 0, 1 );
 if( ( results[3] & 0x80 ) )
  goto iohttpFunc_fleetattackL0;
...
void iohtmlFunc_observe( ReplyDataPtr cnt ) {
	...
	if( ( num = dbUserFleetList( id, &fleetd ) ) <= 0 ) {
		httpString( cnt, "Error while retrieving user fleets list" );
		goto iohttpFunc_observefail;
	}
	
	iohtmlBodyInit( cnt, "Mass observe" );
	...
	if( systemstring ) {
		if( ( sscanf( systemstring, "%d", &system ) <= 0 ) || dbMapRetrieveSystem( system, &systemd ) < 0 ) {
			...
			httpString( cnt, "Unable to observe an Empire System!<br>" );
			goto iohttpFunc_observefail;
		} 
	}
	...
	httpPrintf( cnt, "There are %d unexplored planets in <a href=\"%s&amp;id=%d\" > system %d,%d</a> which will be observed.<br>",count, URLAppend( cnt, "system" ), system, systemd.position & 0xFFFF, systemd.position >> 16);
	if (iohtmlVarsFind( cnt, "agentammount" ) ){
		maind.benefactor[0] = atoi(iohtmlVarsFind( cnt, "agentammount" ));
		if (maind.benefactor[0] < 0) maind.benefactor[0] = 0;
		dbUserMainSet( id, &maind );
	}
	
	
	
	if( ( a = cmdExecSendFleetInfos( id, planetd.id, 0 ) ) < 0 )
	 {
	  if( cmdErrorString )
	   httpPrintf( cnt, "<i>%s</i><br>", cmdErrorString );
	  else
	 ...
	 arl = specopAgentsReadiness( CMD_OPER_OBSERVEPLANET, &maind, 0 );
	 
	 if( !( a ) ){
	  httpPrintf( cnt, "Your %s could reach this system instantaneously,<br>",cmdUnitName[CMD_UNIT_AGENT] );
	  httpPrintf( cnt, "It will cost you %d%% agent readiness and you will need %d agents to observe all planets.<br>", count*(arl >> 16), maind.benefactor[0]);
	 }
	 else{
	  httpPrintf( cnt, "Your %s would reach this system in %d week(s).<br>",cmdUnitName[CMD_UNIT_AGENT], a );
	  httpPrintf( cnt, "It will cost you %d%% agent readiness and you will need %d agents to observe all planets.<br>", count*(arl >> 16), count*maind.benefactor[0]);
	 }

	 httpPrintf( cnt, "You have %d%% ar and %d %s.<br><br>", maind.readiness[2] >> 16, fleetd[0].unit[CMD_UNIT_AGENT], cmdUnitName[CMD_UNIT_AGENT] );
	
	httpPrintf( cnt, "<form action=\"%s\" method=\"POST\"  ><table cellspacing=\"8\">", URLAppend( cnt, "observe" )  );
	httpPrintf( cnt, "<tr><td align=\"right\" nowrap>Ammount of agents to send per planet</td><td><input type=\"text\" name=\"agentammount\" size=\"4\" value=\"%d\"  maxlength=\"6\"> </td>", maind.benefactor[0] );
	httpPrintf( cnt, "<td><input type=\"hidden\" name=\"system\" value=\"%d\" > </td>", system);
    httpString( cnt, "<td align=\"center\"><input type=\"submit\" value=\"Change\" ></td></tr></table></form><br>" );
	
	httpPrintf( cnt, "<a name=\"Dispatch\" href=\"%s&system=%d&dispatch=1\">Dispatch %s</a>", URLAppend( cnt, "operationsend" ), system, cmdUnitName[CMD_UNIT_AGENT] );
	
	iohttpFunc_observefail:
	iohtmlBodyEnd( cnt );
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 else
 {
  ...
  if( dbUserMainRetrieve( planetd.owner, &maind2 ) < 0 )
	   return;
  if( cmdExecFindRelation( maind.empire, maind2.empire, 0, 0 ) == CMD_RELATION_NAP )
	 httpString( cnt, "You have a <font color=\"#3399FF\">non-aggression pact</font> with this empire. Are you sure you want to attack?<br>" );
 ...
 {
  if( fleetd.unit[a] )
   httpPrintf( cnt, "<tr><td align=\"right\">%d %s</td><td><input type=\"text\" maxlength=\"12\" pattern=\"[0-9]+\" name=\"u%d\" size=\"12\"></td></tr>", fleetd.unit[a], cmdUnitName[a], a );
 }
	...
	if( ( b = dbUserSpecOpList( id, &specopd ) ) >= 0 )
	{
	  ...
	  for( a = c = 0 ; a < b ; a++ )
	  {
	   ...
	   if( optype == 0 )
	   {
		...
		else if( opid == CMD_OPER_ESPIONAGE )
		 httpPrintf( cnt, " - <a href=\"%s&amp;id=%d\">See information</a>", URLAppend( cnt, "specinfos" ), a );
		httpString( cnt, "<br>" );
	   }
	   else if( optype == 1 )
	   {
		...
		else if( opid == CMD_SPELL_ENLIGHT )
		{
			  switch (specopd[a].vars[0]){
				case (CMD_ENLIGHT_ENERGY): //between 0 and 15%
				httpPrintf( cnt, " - +%d%% energy production", specopd[a].vars[1] );
				break;
				case (CMD_ENLIGHT_MINERAL): //between 0 and 20%
				httpPrintf( cnt, " - +%d%% mineral production", specopd[a].vars[1] );
				break;
				case (CMD_ENLIGHT_ECTRO): //between 0 and 10%
				httpPrintf( cnt, " - +%d%% ectrolium production", specopd[a].vars[1] );
				break;
				case (CMD_ENLIGHT_CRY): //between 0 and 15%
				httpPrintf( cnt, " - +%d%% crystal production", specopd[a].vars[1] );
				break;
				case (CMD_ENLIGHT_RESEARCH): //between 0 and 10%
				httpPrintf( cnt, " - +%d%% research production", specopd[a].vars[1] );
				break;
				case (CMD_ENLIGHT_SPEED): //between 0 and 25%
				httpPrintf( cnt, " - +%d%% speed", specopd[a].vars[1] );
				break;
				case (CMD_ENLIGHT_BADFR):
				httpPrintf( cnt, " - decrease fr protection by %d%%", specopd[a].vars[1] );
				break;    
...
if ( ( planetstring ) && ( sscanf( planetstring, "%d", &plnid ) == 1 ) ) {
 ...
 httpPrintf( cnt, "<a href=\"%s&amp;type=Operations&amp;request=ajax\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">Operations</a><br>", URLAppend( cnt, "info" ) );
 for( a = 0 ; a < CMD_AGENTOP_NUMUSED ; a++ )
 {
 	//ARTI CODE give agent op all races
 	//if(!((maind.artefacts & ARTEFACT_512_BIT)&&(a == CMD_OPER_NUKEPLANET ))) {
		if (! (roundspec_define[ROUNDSPEC_OBSERVEFORALL] == 1 && a == CMD_OPER_OBSERVEPLANET ) ){
 ...
 httpPrintf( cnt, "Send: <input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" size =\"10\" name=\"sendagents\" value=\"%d\"><br><br>", fleetd.unit[CMD_UNIT_AGENT] );
 if( plnid >= 0 )
 ...
 httpPrintf( cnt, "<a href=\"%s&amp;type=Spells&amp;request=ajax\" rel=\"ajaxpanel\" data-loadtype=\"ajax\">Spells</a><br>", URLAppend( cnt, "info" ) );
 for( a = 0 ; a < CMD_PSYCHICOP_NUMUSED ; a++ )
 {
	 //printf("%d allowed%d \n",a,specopAgentsAllowed( a, maind.raceid ),);
	 //arti 1 phantoms
	  	//if(!((maind.artefacts & ARTEFACT_1_BIT)&&(a == CMD_SPELL_ENLIGHT))) {
 ...
 httpPrintf( cnt, "Send: <input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" size =\"10\" name=\"sendpsychics\" value=\"%d\"><br><br>", fleetd.unit[CMD_UNIT_WIZARD] );
 ...
 httpPrintf( cnt, "%d%% Psychics readiness<br>", maind.readiness[CMD_READY_PSYCH] >> 16 );
 httpPrintf( cnt, "Send: <input type=\"text\" pattern=\"[0-9]+\" maxlength=\"9\" size =\"10\" name=\"sendghosts\" value=\"%d\"><br><br>", fleetd.unit[CMD_UNIT_GHOST] );
 if( plnid >= 0 )
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 if( optype == 0 )
 {
  ...
  if( opid != CMD_OPER_HIGHINFIL && opid != CMD_OPER_ESPIONAGE )
  ...
  if( opid == CMD_OPER_ESPIONAGE )
   goto iohttpFunc_espionage;
    ...
    for( a = 0 ; a < b ; a++ )
    {
     ...
     if(planetd.special[1])
	 {
		if (planetd.special[0] != CMD_BONUS_ANCIENTCIV)
			...
			httpPrintf( cnt, " <img src=\"files?type=image&amp;name=map/ancient.gif\">" );
	 }
     httpPrintf( cnt, "</td></tr>" );
     totals[3] += planetd.population;
     totals[4] += planetd.maxpopulation;
 ...
 else if( optype == 2 )
 {
  ...
  if( opid == CMD_INCANT_SURVEY )
  {
   ...
   for( a = 0 ; a < b ; a++ )
   {
    ...
    d = (int)artefactPrecense( &planetd );
    if( d >= 0 )
     httpPrintf( cnt, "Artefact :  <img src=\"files?type=image&amp;name=artefact/%s\" alt=\"%s\" title=\"%s\"><br>", artefactImage[d], artefactName[d], artefactName[d]  );//ArtefactTable[d]->image, ArtefactTable[d]->name, ArtefactTable[d]->name  );
    if( planetd.special[1] )
    {
	 if (planetd.special[0] != CMD_BONUS_ANCIENTCIV)
		...
		httpPrintf( cnt, "%s : <img src=\"files?type=image&amp;name=map/ancient.gif\"><br>", cmdBonusName[planetd.special[0]] );
   }
 ...
 iohttpFunc_espionage:
 showops(cnt, specopd.factionid);
 iohtmlBodyEnd( cnt );
 return;
...
// we need a table here.
 ...
 if( specop == CMD_OPER_SPYTARGET )
  ...
  httpString( cnt, "Your agents will attempt to place a beacon on target planet, if successful it will remove any dark webs present." );
 else if( specop == CMD_OPER_ESPIONAGE )
  httpString( cnt, "Your diplomats will try to gather information regarding operations and spells, currently affecting the target faction." );
 else if( specop == CMD_OPER_STEALRES )
  httpString( cnt, "Your agents will break on the uguarded storage facilities and steal their resources." );
 ...
 httpPrintf( cnt, "<form action=\"%s&system=%d\" method=\"POST\">", URLAppend( cnt, "operationsend" ),systemd.id );
...
// we need a table here too.
 ...
 if( specop == CMD_SPELL_IRRAECTRO )
  ...
  httpString( cnt, "Philosophers and scientists will try to bring a golden Age of Enlightenment upon your empire." );
 else if( specop == CMD_SPELL_GROWPLANET )
  httpString( cnt, "Your psychics will try to create new land on one of the planets of your empire. <br>The more planets and psychic power you have, the better are the chances." );
 httpString( cnt, "</i><br><br>" );
...
// we need a table here.
 ...
 else if( specop == CMD_INCANT_CALLTOARMS )
  httpString(cnt, "Your forces will attempt to spread propaganda via facebook, convincing population of the targeted planet to <strike>vote for Trump</strike> leave the planet and join your army.");
...
if( ( id = iohtmlIdentify( cnt, 1|2 ) ) < 0 )
 ...
 systemstring = iohtmlVarsFind( cnt, "system" );
 if(systemstring)
	if (sscanf( systemstring, "%d", &system ) <= 0 ) return;
 
if (iohtmlVarsFind( cnt, "Dispatch" ))
	goto MassObserve;
...
if( a ) {
 ...
 MassObserve:
	...
	if( dbMapRetrieveSystem( system, &systemd ) < 0 ) {
		httpString( cnt, "Error retrieving system!<br>" );
		goto iohttpFunc_observefail;
	} else if ( systemd.empire != -1 ) {
		httpString( cnt, "Unable to observe an Empire System!<br>" );
	...
	for(i = 0, num = systemd.indexplanet; i < systemd.numplanets; i++, num++) {
		if( ( dbMapRetrievePlanet( num, &planetd ) < 0 ) ) {
				httpString( cnt, "Planet retrvial error!<br>" );
				goto iohttpFunc_observefail;
		}
		...
		if ( planetd.owner == -1 ) {
			...
			//send agents
			 if( ( a = cmdExecSendAgents( id, x, y, z, CMD_FLEET_ORDER_OBSERVEPLANET, maind.benefactor[0] ) ) < 0 )
				 {
				  if( cmdErrorString )
				   httpPrintf( cnt, "<i>%s</i><br>", cmdErrorString );
				  else
				...
				if( a ) {
					httpPrintf( cnt, "The agents will reach %d,%d:%d  in %d weeks<br>", x,y,z,a );
				} else {
				  if( !( cmdFleetActionNewd[1] & CMD_NEWS_FLAGS_NEW ) )
				   iohtmlNewsString( cnt, cmdFleetActionNewd );
				 }
 ...
 iohttpFunc_observefail:	
 httpPrintf( cnt, "<br><br><a href=\"%s&amp;id=%d\">View system</a>", URLAppend( cnt, "system" ), system );
 iohtmlBodyEnd( cnt );
 return;
}
...
if( ( fund ) && ( sscanf( fund, "%" SCNd64, &addfund ) == 1 ) ) {
	...
	if (addfund > 1000000000000)
		sprintf( fundstring, "<i>You are trying to fund too much</i><br><br>" );
	else{
...
if( dbUserMainSet( id, &maind ) < 0 ) {
 ...
 httpPrintf( cnt, "Increase fundings<br><form action=\"%s\" method=\"POST\"><input type=\"text\" pattern=\"[0-9]+\" maxlength=\"12\" name=\"fund\" size=\"12\">&nbsp;<input type=\"submit\" value=\"Fund\"></form><br>Current research fundings : %lld<br><br>", URLAppend( cnt, "research" ), (long long)maind.fundresearch );
...
if( typestring ) {
	...
	if (maind.benefactor[4] != 1){ //to ban a user's mail
		...
		httpString( cnt, "</td></tr><tr><td>Message</td><td><textarea name=\"mail\" rows=\"10\" cols=\"60\">" );
		if( ( mailstring ) ) {
			if( ( maild.text = malloc( 2*MAIL_MAX ) ) ) {
				a = 0;
				if((cnt->session)->dbuser) {
		...
		httpString( cnt, "You were banned from using your mail.<br><br>" );
	}
}
...
if (type && round == ticks.round ){
int header = 0;
	for (int i =0; i < ROUNDSPEC_NUMUSED; i++)
	{	
		if (roundspec_define[i] != 0){
			if (!header){
				httpString(cnt, "<b>Round specialties:</b><br>");
				++header;
			}
		httpPrintf(cnt, "<img src=\"files?type=image&amp;name=artefact/%s\"> %s<br>", RoundSpecImage[i], RoundSpecName[i]);	
		}
	}
}
