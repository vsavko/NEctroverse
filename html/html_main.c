...
if( ( NULL == (cnt->session)->dbuser ) || ( NULL == (cnt->session)->sid ) ) {	// -somehow its allways NULL
	goto FAIL;
} else {
...
//httpString( cnt, "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0 \">");
...
httpString( cnt, "<p hidden id=\"windowWidth\" name = \"windowWidth\" ></p>");
httpString( cnt, "<script> var width = window.innerWidth;  document.getElementById(\"windowWidth\").innerHTML = width; </script>" );

//printf("Test var %s\n" , iohtmlVarsFind( cnt, "width" ));
//printf("Test inner %s\n" , iohtmlVarsFind( cnt, "window.innerWidth" ));
//printf("Test p %s\n" , iohtmlVarsFind( cnt, "windowWidth" ));
//httpString( cnt, "<script> alert(width); </script>" );
//printf("Test var %s\n" , iohtmlVarsFind( cnt, "width" ));
//printf("Test inner %s\n" , iohtmlVarsFind( cnt, "window.innerWidth" ));
//printf("Test p %s\n" , iohtmlVarsFind( cnt, "windowWidth" ));
...
httpString( cnt, "<tr><td align=\"center\"><img style=\"display:block;\" src=\"files?type=image&amp;name=ectro_03.jpg\" width=350 height=80  alt=\"ectTop\"></td></tr>" );
httpString( cnt, "<tr><td><table width=\"100%\" border=\"0\" cellpadding=\"0\"cellspacing=\"0\"><tr><td background=\"files?type=image&amp;name=ectro_12.jpg\" width=\"45%\"></td>" );
httpString( cnt, "<td align=\"center\" width=\"10%\"><img style=\"display:block;\" src=\"files?type=image&amp;name=ectro_06.jpg\" width=\"450\"height=\"75\"></td>" );
httpString( cnt, "<td background=\"files?type=image&amp;name=ectro_12.jpg\"width=\"45%\"></td></tr></table>" );
httpString( cnt, "<tr><td align=\"center\"><img style=\"display:block;\" src=\"files?type=image&amp;name=ectro_09.jpg\" width=\"660\" height=\"100\"></td></tr>" );
httpString( cnt, "<tr><td background=\"files?type=image&amp;name=ectro_12.jpg\" align=\"center\"><table width=\"660\" height=\"75\"" );
httpString( cnt, "border=\"0\" cellpadding=\"0\" cellspacing=\"0\"></td></tr><tr><td background=\"files?type=image&amp;name=ectro_13.jpg\"" );
...
if( !( flags == FMENU_GUIDE ) ) {
	httpString( cnt, " | " );
	httpPrintf( cnt, "<a href=\"%s\">Guide</a>", URLAppend( cnt, "guide" ) );
}
...
if( !( flags == FMENU_SERVER ) ) {
	httpString( cnt, " | " );
	httpPrintf( cnt, "<a href=\"%s\">Server</a>", URLAppend( cnt, "status" ) );
}
...
httpString( cnt, "<tr><td background=\"files?type=image&amp;name=ectro_12.jpg\" valign=\"middle\"></td></tr>" );
...
void httpPrintBox(ReplyDataPtr cnt, char * heading, char *data){
	
httpString( cnt,"<div class=\"display_box_wrapper\">");
httpString( cnt,"<div class=\"display_box\">");
httpString( cnt,"<div class=\"display_box_header\">");
httpString( cnt,"<div class=\"display_box_header_left\"></div>");
httpString( cnt,"<div class=\"display_box_header_center\">");
httpPrintf( cnt,"<div class=\"display_box_header_center_content\">%s</div>",heading);
httpString( cnt,"</div>");
httpString( cnt,"<div class=\"display_box_header_right\"></div>");
httpString( cnt,"</div>");
httpString( cnt,"<div class=\"display_box_content\">");
httpString( cnt,"<div class=\"display_box_content_left\"></div>");
httpString( cnt,"<div class=\"display_box_content_center\">");
httpPrintf(cnt, "<slot name=\"content\">%s</slot>", trimwhitespace(data));
httpString( cnt,"</div>");
...
void iohtmlFunc_guide( ReplyDataPtr cnt ) {
	ConfigArrayPtr settings;
	struct stat stdata;
	char *data;
	char DIRCHECKER[PATH_MAX];
...
if( stat( DIRCHECKER, &stdata ) != -1 ) {
	if( ( data = malloc( stdata.st_size + 1 ) ) ) {
		...
		if( ( file = fopen( DIRCHECKER, "rb" ) ) ) {
			if( ( fread( data, 1, stdata.st_size, file ) < 1 ) && ( stdata.st_size ) ) {
				error( "Failure reading guide file." );
			} else {
				//httpString( cnt, trimwhitespace(data) );
				httpPrintBox(cnt, "Guide", data);
			}
			fclose( file );
		}
		free( data );
...
settings = GetSetting( "HTTP Text" );
sprintf( DIRCHECKER, "%s/GettingStarted.html", settings->string_value );
if( stat( DIRCHECKER, &stdata ) != -1 ) {
	if( ( data = malloc( stdata.st_size + 1 ) ) ) {
		data[stdata.st_size] = 0;
		if( ( file = fopen( DIRCHECKER, "rb" ) ) ) {
			if( ( fread( data, 1, stdata.st_size, file ) < 1 ) && ( stdata.st_size ) ) {
				...
				httpPrintBox(cnt, "Getting Started", data);
			}
			fclose( file );
		}
		free( data );
...
//printf("%s",settings->string_value );
if( stat( DIRCHECKER, &stdata ) != -1 ) {
	if( ( data = malloc( stdata.st_size + 1 ) ) ) {
		data[stdata.st_size] = 0;
		if( ( file = fopen( DIRCHECKER, "rb" ) ) ) {
...
if( ( name ) && ( pass ) ) {
	...
	if( ( file = fopen( DIRCHECKER, "a" ) ) ) {
		...
		//if there is a reverse proxy by apache, get the ip from the header
		if ((header = iohtmlHeaderFind( cnt, "X-Forwarded-For" )) != NULL){
			fprintf( file, "IP %s;\n", header);
		}
		else{
...
#define REGISTER_DISABLE FALSE

//use to disable mass registration/ e-mail sending from one 
typedef struct IPRegister IPRegister;

typedef struct IPRegister{
	time_t time;
	char *ip;
	int tries;
	IPRegister * next;
...
void iohtmlFunc_register( ReplyDataPtr cnt ) {
	...
	struct stat stdata;
	char *data;
	char *name, *pass, *faction, *race, *empire, *fampass, *rules, *email, *delete, *confirmationCode, *tmpCode, *currentIp = NULL;
	char *token = NULL;
	dbUserMainDef maind;
	#if FACEBOOK_SUPPORT
	FBUserDef fbdata;
	...
	static IPRegisterPtr *registeredIpPtr = 0;
	IPRegisterPtr *tmpRegisteredIpPtr;
	bool allowRegister = true, ipFound = false;
	time_t registerTime = 0;
	int blockTime = 600; //number of seconds to block registration for a user after 3 attempts
...
email = iohtmlVarsFind( cnt, "email" );
delete = iohtmlVarsFind( cnt, "delete" );
confirmationCode = iohtmlVarsFind( cnt, "confirmationCode" );
tmpCode = iohtmlVarsFind( cnt, "tmpCode" );
...
char * headerTmp = iohtmlHeaderFind( cnt, "X-Forwarded-For" );

if (headerTmp == NULL){ //for reverse proxy
	if( (cnt->connection)->addr->sa_family == AF_INET )
		currentIp = strdup(inet_ntoa( ((struct sockaddr_in *)(cnt->connection)->addr)->sin_addr ));
}
else
	currentIp = strdup(headerTmp);


//printf("%s", currentIp);

//check register tries
if (registeredIpPtr == 0){
	registeredIpPtr = null;
}

tmpRegisteredIpPtr = registeredIpPtr;

while(tmpRegisteredIpPtr != null){
	//clear expired ip registers
	if(time(&now) - registeredIpPtr->time > blockTime){
		registeredIpPtr = tmpRegisteredIpPtr->next;
		free(tmpRegisteredIpPtr->ip);
		free(tmpRegisteredIpPtr);
		tmpRegisteredIpPtr = registeredIpPtr;
	}
	else
		tmpRegisteredIpPtr = tmpRegisteredIpPtr->next;
}

tmpRegisteredIpPtr = registeredIpPtr;

//search linked list of IPRegister structs to see if current user has recently registered
do{
	if (tmpRegisteredIpPtr == null) break;
	//error("ip %s tries %d",tmpRegisteredIpPtr->ip, tmpRegisteredIpPtr->tries);
	if(strcmp(currentIp, tmpRegisteredIpPtr->ip) == 0){
		ipFound = true;
		if (tmpRegisteredIpPtr->tries > 2){
			allowRegister = false;
			registerTime = tmpRegisteredIpPtr->time;
		}
	}
	if (tmpRegisteredIpPtr->next != null)
		tmpRegisteredIpPtr = tmpRegisteredIpPtr->next;
} while (tmpRegisteredIpPtr->next != null);
...
if(delete != null && id > 0) {
	cmdUserDelete( id );
	httpPrintf( cnt, "Your account: \"%s\" was deleted!<br>",((cnt->session)->dbuser)->name );
	(cnt->session)->dbuser = null;
	goto END;
}
...
if( race ) {
	...
	if (confirmationCode != null){	//email code entered
		if (strcmp(confirmationCode, tmpCode)!= 0){
			...
			httpString( cnt, cmdErrorString ? cmdErrorString  : "Wrong confirmation code!" );
			httpString( cnt, "<br><br>" );
			httpPrintf( cnt, "<a href=\"%s&amp;rule_agree=true\">Try Again.</a>", URLAppend( cnt, "register" ) );
			goto END;
		}
	...
	else{ //email sent, code to be entered

		if( ( id = cmdEmailCheckNewUser( name, pass, faction, email, &tmpCode, currentIp ) ) < 0 ) {
			httpString( cnt, cmdErrorString ? cmdErrorString  : "Error encountered while checking user" );
			httpString( cnt, "<br><br>" );
			httpPrintf( cnt, "<a href=\"%s&amp;rule_agree=true\">Try Again.</a>", URLAppend( cnt, "register" ) );
			goto END;
		}
			...
			if (ipFound == false){
				if (registeredIpPtr == null){
					tmpRegisteredIpPtr = malloc(sizeof(IPRegisterPtr));
					registeredIpPtr = tmpRegisteredIpPtr;
				}
				else{
					tmpRegisteredIpPtr = registeredIpPtr;
					while(tmpRegisteredIpPtr->next != null);
					tmpRegisteredIpPtr->next = malloc(sizeof(IPRegisterPtr));
					tmpRegisteredIpPtr = tmpRegisteredIpPtr->next;
				}
					
				tmpRegisteredIpPtr->ip = strdup(currentIp);
				tmpRegisteredIpPtr->time = time( &now );
				tmpRegisteredIpPtr->tries = 1;
				tmpRegisteredIpPtr->next = null;
			}
			else{
				tmpRegisteredIpPtr->tries += 1;
			...
			//copy allready inputted code to the registration page after email confirmation, otherwise it gets deleted
			if( rules != NULL ) httpPrintf( cnt, "<input type=\"hidden\" name=\"rule_agree\" value=\"%s\">", rules) ;
			if( faction != NULL ) httpPrintf( cnt, "<input type=\"hidden\" name=\"faction\" value=\"%s\">", faction) ;
			if( pass != NULL ) 	httpPrintf( cnt, "<input type=\"hidden\" name=\"pass\" value=\"%s\">", pass );
			if( name != NULL ) 	httpPrintf( cnt, "<input type=\"hidden\" name=\"name\" value=\"%s\">", name );
			if( email != NULL ) 	httpPrintf( cnt, "<input type=\"hidden\" name=\"email\" value=\"%s\">", email );
			httpString( cnt, "<br><input type=\"submit\" value=\"OK\"></form>" );
			free(tmpCode);
			goto END;
	...
	httpPrintf( cnt, "E-mail : %s<br>", ((cnt->session)->dbuser)->email );
	settings = GetSetting( "Directory" );
	...
	if( ( file = fopen( COREDIR, "a" ) ) ) {
		fprintf( file, "%d|%s|%llu|%s\n", id,currentIp,(long long)(time( &now )),((cnt->session)->dbuser)->email);
		fclose( file );
	}
	...
	if (allowRegister == true){
		...
		httpString( cnt, "<br>E-mail<br><input type=\"email\" name=\"email\"><br>" );
		httpString( cnt, "<br><input type=\"submit\" value=\"OK\"></form>" );
	}
	else{
		//just tried to register > 3 times
		int remainTime = blockTime - (time(&now) - registerTime);
		httpPrintf( cnt, "You have tried to register too many times, wait %d seconds.<br>", remainTime);
		//successfully registered from the same ip allready - wait 3 days
		//admin will be automatically notified
	}
...
httpPrintf( cnt, "<br><a href=\"%s&delete=true\">Delete your account</a>", URLAppend( cnt, "register" ) );
