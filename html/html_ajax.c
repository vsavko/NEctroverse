...
if( ( typestring ) && ( refer ) ) {
	...
	if( strcmp(typestring,"ticker") == CMP_TRUE ) {
		...
		if( split ) {
			httpPrintf( cnt, "<page>%s</page>", split[num-1] );
		}
		...
		if( strcmp(split[num-1],"status") == CMP_TRUE ) {
			getsys_infos( &pinfod );
		...
		httpString( cnt, "</ressources>" );
		httpString( cnt, "</header>" );
		httpPrintf( cnt, "<%s>", split[num-1] );
		//End Header block -- Start HQ block
		if( !strcmp(split[num-1],"hq") ) {
		...
		//End HQ block -- Start Council block
		} else if( strcmp(split[num-1],"council") == CMP_TRUE ) {
			...
			httpPrintf( cnt, "<form name=\"cancelunit\" id=\"cancel_units\" action=\"%s\"><table>", URLAppend( cnt, "cancelbuild" ) );
			for( a = c = 0 ; a < numbuild ; a++ ) {
				if( !( build[a].type >> 16 ) )
					continue;
...
if( refer ) {
	...
	httpString( cnt, "\n" );
	if( ( strlen( split[num-1] ) == 0 ) || ( strcmp(split[num-1],"faq") == 0 ) || ( strcmp(split[num-1],"halloffame") == 0 ) || ( strcmp(split[num-1],"gettingstarted") == 0 ) || ( strcmp(split[num-1],"register") == 0 ) || ( strcmp(split[num-1],"login") == 0 ) || ( strcmp(split[num-1],"logout") == 0 ) || ( strcmp(split[num-1],"status") == 0 ) ) {
		httpString( cnt, "\t\t\tupdatehtml(\"u_online\",u_online);\n" );
		httpString( cnt, "\t\t\tupdatehtml(\"u_activated\",u_activated);\n" );
	}
	
	if( strcmp(split[num-1],"hq") == CMP_TRUE ) {
		httpString( cnt, "\t\t\tupdatehtml(\"hqweeks\",week);\n" );
		httpString( cnt, "\t\t\tupdatehtml(\"hqyears\",year);\n" );
		httpString( cnt, "\t\t\tupdatehtml(\"hqStartTime\",start);\n" );
	} else if( strcmp(split[num-1],"status") == CMP_TRUE ) {
	...
	httpString( cnt, "\t\t\tupdatehtml(\"energyinterest\",\"+\" + energyinterest);\n" );
