...
if( type == NULL ) {
	...
	goto ARTEFACTS;
} else if( strcmp( type, "Operations" ) == 0 ) {
	iohtmlBodyInit( cnt, "%s: Operations Info", settings->string_value );
	goto OPERATIONS;
} else if( strcmp( type, "Spells" ) == 0 ) {
	iohtmlBodyInit( cnt, "%s: Spells Info", settings->string_value );
	goto SPELLS;
} else if( strcmp( type, "Incantations" ) == 0 ) {
	iohtmlBodyInit( cnt, "%s: Incantations Info", settings->string_value );
	goto INCANATIONS;
} else {
	INVALID:
...
httpPrintf( cnt, "<tr><td>Name</td><td>Operations research required</td><td>AR cost</td><td>Difficulty</td><td>Stealth</td></tr>");
for ( a = 0 ; a < CMD_AGENTOP_NUMUSED; a++){
	httpPrintf( cnt, "<tr><td><b>%s<b></td><td>%d</td><td>%g</td><td>%g</td><td>%s</td></tr>" , cmdAgentopName[a], cmdAgentopTech[a], cmdAgentopReadiness[a], cmdAgentopDifficulty[a], (cmdAgentopStealth[a] == 1) ? "yes" : "no");
	httpPrintf( cnt, "<tr><td>%s</td></tr><tr><td></td></tr>", OperationsDescription[a]);
...
SPELLS:
httpPrintf(cnt, "<table>");
httpPrintf( cnt, "<tr><td>Name</td><td>Culture research required</td><td>PR cost</td><td>Difficulty</td><td>Self-spell</td></tr>");
for ( a = 0 ; a < CMD_PSYCHICOP_NUMUSED; a++){
	httpPrintf( cnt, "<tr><td><b>%s<b></td><td>%d</td><td>%g</td><td>%g</td><td>%s</td></tr>" , cmdPsychicopName[a], cmdPsychicopTech[a], cmdPsychicopReadiness[a], cmdPsychicopDifficulty[a], (cmdPsychicopFlags[a] == 2) ? "yes" : "no");
	httpPrintf( cnt, "<tr><td>%s</td></tr><tr><td></td></tr>", SpellsDescription[a]);
...
INCANATIONS:
httpPrintf(cnt, "<table>");
httpPrintf( cnt, "<tr><td>Name</td><td>Culture research required</td><td>Difficulty</td><td>PR cost</td><td>Stealth</td><td>Self-spell</td></tr>");
for ( a = 0 ; a < CMD_GHOSTOP_NUMUSED; a++){
	httpPrintf( cnt, "<tr><td><b>%s<b></td><td>%d</td><td>%g</td><td>%d</td><td>%s</td><td>%s</td></tr>" , cmdGhostopName[a], cmdGhostopTech[a], cmdGhostopReadiness[a], cmdGhostopDifficulty[a], ((cmdGhostopFlags[a]&1) == 1) ? "yes" : "no", ((cmdGhostopFlags[a]&2) == 2) ? "yes" : "no");
	httpPrintf( cnt, "<tr><td>%s</td></tr><tr><td></td></tr>", IncanationsDescription[a]);
...
for( a = 0; a < CMD_RACE_NUMUSED ; a++) {
	httpString( cnt,"<font color=\"white\">");
...
for( a = table = 0; a < CMD_BLDG_NUMUSED+1; a++, table++ ) {
	...
	if (a == CMD_BUILDING_SATS){
		httpPrintf( cnt, "<tr><td><span class=\"genblue\">Air Attack: </span></td><td>%d</td></tr>", CMD_SATS_ATTACK );
		httpPrintf( cnt, "<tr><td><span class=\"genblue\">Air Defense: </span></td><td>%d</td></tr>", CMD_SATS_DEFENCE );
	}
	
	if (a == CMD_BUILDING_SHIELD){
		httpPrintf( cnt, "<tr><td><span class=\"genblue\">Damage absorb: </span></td><td>%d</td></tr>", CMD_SHIELD_ABSORB );
	}
...
httpPrintf( cnt, "There are %d Artefacts in total.<br>", ARTEFACT_NUMUSED );
httpString( cnt, "<table class=\"left\">" );
/*
for( a = b = 0; a < ARTEFACT_NUMUSED; a++ ) {
	httpString( cnt, "<tr>" );
	httpPrintf( cnt, "<tr><img src=\"files?type=image&name=artefact/%s\"></td>", artefactImage[a] );//ArtefactTable[a]->image );
	httpPrintf( cnt, "<td align=\"left\">%s</td>",  artefactName[a]; //ArtefactTable[a]->name ); //artefactImage[b], artefactName[b], artefactName[b], artefactDescription[b]
